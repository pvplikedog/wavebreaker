...
namespace Pathfinding
{
	...
	public class ABPath : Path
    {
	    ...
	    ///     The <see cref="endNode" /> and <see cref="endPoint" /> will be modified and be set to the node which ends up being
        ...
        protected override void Prepare()
        {
            ...
            if (pathNNConstraint != null) pathNNConstraint.SetStart(startNNInfo.node);
        ...
        protected override void CalculateStep(long targetTick)
        {
            ...
            while (CompleteState == PathCompleteState.NotCalculated)
            {
                ...
                if (currentR.H < partialBestTarget.H) partialBestTarget = currentR;
            ...
            if (CompleteState == PathCompleteState.Complete)
                Trace(currentR);
            else if (calculatePartial && partialBestTarget != null) CompletePartial(partialBestTarget);
        ...
        protected override string DebugString(PathLog logMode)
        {
            ...
            if (!error && logMode == PathLog.Heavy)
            {
                ...
                text.Append("\nStart Node");
                text.Append("\n	Point: ");
                text.Append(startPoint.ToString());
                text.Append("\n	Graph: ");
        ...
        ///     A simple and quite slow (well, compared to more optimized algorithms) algorithm first finds the closest path
        ///     segment (from <see cref="vectorPath)" /> and then returns
        ///     the direction to the next point from there. The direction is not normalized.
        ///     Returns: Direction to move from a point, returns Vector3.zero if <see cref="vectorPath" /> is null or has a length
        ///     of 0
        ///     Deprecated:
        /// </summary>
        [Obsolete]
        ...
        {
            if (vectorPath == null || vectorPath.Count == 0) return Vector3.zero;

            if (vectorPath.Count == 1) return vectorPath[0] - point;
                ...
                if (gridNode != gridNode2 && gridNode2 != null && gridNode.GraphIndex == gridNode2.GraphIndex)
                {
                    ...
                    switch (gridGraph.neighbours)
                    {
                        case NumNeighbours.Four:
                            if ((x1 == x2 && Math.Abs(z1 - z2) == 1) || (z1 == z2 && Math.Abs(x1 - x2) == 1))
