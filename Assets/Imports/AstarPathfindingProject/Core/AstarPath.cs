...
public class AstarPath : VersionedMonoBehaviour
{
    ...
    public static readonly Version Version = new(4, 2, 17);
    ...
    [FormerlySerializedAs("astarData")] public AstarData data;
    ...
    ///     For example if <see cref="debugMode" /> is set to G, this value will determine when the node will be completely
    ///     red.
    ///     Note: Only relevant in the editor
    ...
    ///     For example if <see cref="debugMode" /> is set to G, this value will determine when the node will be completely
    ///     green.
    ///     For the penalty debug mode, the nodes will be colored green when they have a penalty less than
    ///     <see cref="debugFloor" /> and red
    ...
    ///     Note: Only relevant in the editor
    ///     TODO: Add a showOnlyLastPath flag to indicate whether to draw every node or only the ones visited by the latest
    ///     path.
    /// </summary>
    ...
    ///     When searching for the nearest node to a point, this is the limit (in world units) for how far away it is allowed
    ///     to be.
    ///     This is relevant if you try to request a path to a point that cannot be reached and it thus has to search for
    ///     the closest node to that point which can be reached (which might be far away). If it cannot find a node within this
    ///     distance
    ...
    ///     If you disable this, you will have to call \link Scan AstarPath.active.Scan() \endlink yourself to enable
    ///     pathfinding.
    ...
    ///     Additional searches will normally only be done on the graph which in the first fast search seemed to have the
    ///     closest node.
    ///     With this setting on, additional searches will be done on all graphs since the first check is not always completely
    ///     accurate.\n
    ///     More technically: GetNearestForce on all graphs will be called if true, otherwise only on the one graph which's
    ///     GetNearest search returned the best node.\n
    ///     Usually faster when disabled, but higher quality searches when enabled.
    ///     When using a a navmesh or recast graph, for best quality, this setting should be combined with the
    ///     Pathfinding.NavMeshGraph.accurateNearestNode setting set to true.
    ...
    ///     The first graph which has a node closer than <see cref="prioritizeGraphsLimit" /> will be chosen instead of
    ...
    /// </summary>
    [SerializeField] protected string[] tagNames;
    ...
    ///     If 0 is used, the pathfinding algorithm will be reduced to dijkstra's algorithm. This is equivalent to setting
    ///     <see cref="heuristic" /> to None.
    ///     If a value larger than 1 is used the pathfinding will (usually) be faster because it expands fewer nodes, but the
    ///     paths may no longer be the optimal (i.e the shortest possible paths).
    ...
    ///     Multithreading puts pathfinding in another thread, this is great for performance on 2+ core computers since the
    ///     framerate will barely be affected by the pathfinding at all.
    ...
    ///     If you use more threads than the number of cores on the computer it is mostly just wasting memory, it will not run
    ///     any faster.
    ///     The extra memory usage is not trivially small. Each thread needs to keep a small amount of data for each node in
    ///     all the graphs.
    ///     It is not the full graph data but it is proportional to the number of nodes.
    ///     The automatic settings will inspect the machine it is running on and use that to determine the number of threads so
    ///     that no memory is wasted.
    ///     The exception is if you only have one (or maybe two characters) active at time. Then you should probably just go
    ///     with one thread always since it is very unlikely
    ///     that you will need the extra throughput given by more threads. Keep in mind that more threads primarily increases
    ///     throughput by calculating different paths on different
    ///     threads, it will not calculate individual paths any faster.
    ///     Note that if you are modifying the pathfinding core scripts or if you are directly modifying graph data without
    ///     using any of the
    ///     safe wrappers (like <see cref="AddWorkItem)" /> multithreading can cause strange errors and pathfinding stopping to
    ///     work if you are not careful.
    ///     For basic usage (not modding the pathfinding core) it should be safe.
    ///     Note: WebGL does not support threads at all (since javascript is single-threaded) so no threads will be used on
    ///     that platform.
    ...
    ///     <see cref="graphUpdateBatchingInterval)" />.
    ...
    ///     If <see cref="batchGraphUpdates" /> is true, this defines the minimum number of seconds between each batch of graph
    ///     updates.
    ///     This can have a positive impact on pathfinding throughput since the pathfinding threads do not need
    ///     to be stopped as often, and it reduces the overhead per graph update.
    ///     All graph updates are still applied however, they are just batched together so that more of them are
    ...
    {
        get => graphUpdateBatchingInterval;
	...
	/// </summary>
	[NonSerialized] public PathHandler debugPathData;

    /// <summary>The path ID to debug using gizmos</summary>
    [NonSerialized] public ushort debugPathID;
    ...
    /// </summary>
    [NonSerialized] private bool isScanningBacking;
    ...
    ///     When using multithreading, this will be the number of threads, if not using multithreading it is always 1 (since
    ///     only 1 coroutine is used).
    ...
    ///     Note: This uses info about if threads are running right now, it does not use info from the settings on the A*
    ///     object.
    /// </summary>
    ...
    ///     <see cref="RegisterSafeUpdate" /> or <see cref="AddWorkItem" />.
    /// </summary>
    ...
    ///     Such as normal graph updates, navmesh cutting and anything added by <see cref="RegisterSafeUpdate" /> or
    ...
    ///     Such as normal graph updates, navmesh cutting and anything added by <see cref="RegisterSafeUpdate" /> or
    ///     <see cref="AddWorkItem" />.
    ///     In contrast to <see cref="IsAnyWorkItemInProgress" /> this is only true when work item code is being executed, it
    ///     is not
    ...
    ///     This is called at the start of the Awake call, right after <see cref="active" /> has been set, but this is the only
    ...
    ///     settings can only be changed in Awake
    ...
    /// <summary>
    ///     Called for each path before searching. Be careful when using multithreading since this will be called from a
    ///     different thread.
    /// </summary>
    public static OnPathDelegate OnPathPreSearch;

    /// <summary>
    ///     Called for each path after searching. Be careful when using multithreading since this will be called from a
    ///     different thread.
    /// </summary>
    ...
    /// <summary>
    ///     Called after scanning. This is called before applying links, flood-filling the graphs and other post
    ///     processing.
    /// </summary>
    ...
    ///     Note: This callback will be cleared every time it is called, so if you want to register to it repeatedly, register
    ///     to it directly on receiving the callback as well.
    /// </summary>
    public static Action On65KOverflow;

    /// <summary>Deprecated:</summary>
    [Obsolete] public Action OnGraphsWillBeUpdated;

    /// <summary>Deprecated:</summary>
    [Obsolete] public Action OnGraphsWillBeUpdated2;
    ...
    public string[] GetTagNames()
    {
        if (tagNames == null || tagNames.Length != 32)
        {
            ...
            for (var i = 0; i < tagNames.Length; i++) tagNames[i] = "" + i;
    ...
    ///     This is useful if you want to do changes to the graphs in the editor outside of play mode, but cannot be sure that
    ///     the graphs have been deserialized yet.
    ///     In play mode this method does nothing.
    /// </summary>
    public static void FindAstarPath()
    {
        if (Application.isPlaying) return;
        if (active == null) active = FindObjectOfType<AstarPath>();
            ...
            if (graphs[i] != null && graphs[i].drawGizmos)
                graphs[i].GetNodes(node =>
                {
                    if (node.Walkable && (ignoreSearchTree ||
                                          GraphGizmoHelper.InSearchTree(node, debugPathData, debugPathID)))
                    {
    ...
    private readonly RetainedGizmos gizmos = new();
    ...
    private void OnDrawGizmos()
    {
        ...
        if (active != this || graphs == null) return;
        ...
        {
            if (showNavGraphs && !manualDebugFloorRoof) RecalculateDebugLimits();
	...
	private void Update()
    {
        ...
        // when not scanning
        if (!isScanning) PerformBlockingActions();
    ...
    ///     Deprecated: This method has been moved. Use the method on the context object that can be sent with work item
    ///     delegates instead
    ///     <code>
    ...
    /// <summary>
    ///     If a WorkItem needs to have a valid flood fill during execution, call this method to ensure there are no pending
    ///     flood fills.
    ///     Deprecated: This method has been moved. Use the method on the context object that can be sent with work item
    ///     delegates instead
    ///     <code>
    ...
    ///     When using more threads than one, calling this often might decrease pathfinding performance due to a lot of idling
    ///     in the threads.
    ///     Not performance as in it will use much CPU power, but performance as in the number of paths per second will
    ///     probably go down
    ...
    public void AddWorkItem(AstarWorkItem item)
    {
        ...
        if (!workItemLock.Held) workItemLock = PausePathfindingSoon();

#if UNITY_EDITOR
        // If not playing, execute instantly
        if (!Application.isPlaying) FlushWorkItems();
#endif
    }
    ...
    ///     This function is useful if you are limiting graph updates, but you want a specific graph update to be applied as
    ///     soon as possible regardless of the time limit.
    ///     Note that this does not block until the updates are done, it merely bypasses the <see cref="batchGraphUpdates" />
    ///     time limit.
    ...
    ///     If batchGraphUpdates is set, we want to keep some space between them to let pathfinding threads running and then
    ///     calculate all queued calls at once
    /// </summary>
    ...
    public void UpdateGraphs(GraphUpdateObject ob)
    {
        ...
        {
            if (!graphUpdateRoutineRunning) StartCoroutine(DelayedGraphUpdate());
        }
        else
        {
    ...
    ///     This will force the pathfinding threads to finish calculating the path they are currently calculating (if any) and
    ///     then pause.
    ///     When all threads have paused, graph updates will be performed.
    ///     Warning: Using this very often (many times per second) can reduce your fps due to a lot of threads waiting for one
    ///     another.
    ...
    ///     another.
    ///     But you probably wont have to worry about that
    ///     Note: This is almost (note almost) identical to <see cref="FlushGraphUpdates" />, but added for more descriptive
    ///     name.
    ...
    /// </summary>
    /// <param name="unblockOnComplete">
    ///     If true, pathfinding will be allowed to start running immediately after completing all
    ///     work items.
    /// </param>
    /// <param name="block">
    ...
    ///     Returns: An int specifying how many threads to use, 0 means a coroutine should be used for pathfinding instead of a
    ///     separate thread.
    ///     If count is set to Automatic it will return a value based on the number of processors and memory for the current
    ///     system.
    ...
    protected override void Awake()
    {
        ...
        if (OnAwakeSettings != null) OnAwakeSettings();
        ...
        if (scanOnStartup && (!data.cacheStartup || data.file_cachedStartup == null)) Scan();
    }
    ...
    private void InitializePathProcessor()
    {
        ...
        {
            if (euclideanEmbedding.dirty) euclideanEmbedding.RecalculateCosts();
        };

        if (multithreaded) graphUpdates.EnableMultithreading();
    }
    ...
    internal void VerifyIntegrity()
    {
        ...
        if (data == null) throw new NullReferenceException("data is null... A* not set up correctly?");
    ...
    ///     Also calls OnEnable for the <see cref="colorSettings" /> and initializes data.userConnections if it wasn't
    ///     initialized before
    ...
    ///     Deprecated: Deprecated: Not meaningful anymore. The HierarchicalGraph takes care of things automatically behind the
    ///     scenes
    /// </summary>
    ...
    ///     HierarchicalGraph class takes care of things automatically behind the scenes now.
    /// </summary>
    ...
    ///     This is to be called after the node has been disconnected from the graph so that it cannot be reached from any
    ///     other nodes.
    ///     It should only be called during graph updates, that is when the pathfinding threads are either not running or
    ///     paused.
    ...
    /// graphLock.Release();
    /// </code>
    ///     Returns: A lock object. You need to call <see cref="Pathfinding.PathProcessor.GraphUpdateLock.Release" /> on that
    ///     object to allow pathfinding to resume.
    ...
    {
        if (graphToScan == null) throw new ArgumentNullException();
    ...
    public IEnumerable<Progress> ScanAsync(NavGraph[] graphsToScan = null)
    {
        ...
        if (graphsToScan == null) yield break;

        if (isScanning) throw new InvalidOperationException("Another async scan is already running");
        ...
        if (Time.frameCount != startFrame)
            throw new Exception("Async scanning can only be done in the pro version of the A* Pathfinding Project");

        if (OnPreScan != null) OnPreScan(this);
        ...
        for (var i = 0; i < graphsToScan.Length; i++)
        {
            ...
            var maxp = Mathf.Lerp(0.1F, 0.8F, (i + 0.95F) / graphsToScan.Length);
    ...
    private IEnumerable<Progress> ScanGraph(NavGraph graph)
    {
        ...
        foreach (var p in ((IGraphInternals)graph).ScanInternal()) yield return p.MapTo(0, 0.95f);
    ...
    ///     it will calculate most of the paths in the queue (only most if using multithreading, all if not using
    ///     multithreading).
    ...
    ///     \throws Exception if pathfinding is not initialized properly for this scene (most likely no AstarPath object
    ///     exists)
    ///     or if the path has not been started yet.
    ///     Also throws an exception if critical errors occur such as when the pathfinding threads have crashed (which should
        ...
        if (path.PipelineState < PathState.ReturnQueue)
        {
            if (active.IsUsingMultithreading)
                ...
                while (path.PipelineState < PathState.ReturnQueue)
                {
                    ...
                    {
                        waitForPathDepth--;
                        throw new Exception("Critical error. Path Queue is empty but the path state is '" +
                                            path.PipelineState + "'");
                    }
    ...
    ///     but performance as in the number of paths per second will probably go down (though your framerate might actually
    ///     increase a tiny bit)
    ///     You should only call this function from the main unity thread (i.e normal game code).
    ...
    ///     Deprecated: Use <see cref="AddWorkItem(System.Action)" /> instead. Note the slight change in behavior (mentioned
    ///     above).
    /// </summary>
    [Obsolete(
    ...
    /// <param name="pushToFront">
    ///     If true, the path will be pushed to the front of the queue, bypassing all waiting paths and making it the next path
    ///     to be calculated.
    ///     This can be useful if you have a path which you want to prioritize over all others. Be careful to not overuse it
    ///     though.
    ///     If too many paths are put in the front of the queue often, this can lead to normal paths having to wait a very long
    ///     time before being calculated.
    /// </param>
    public static void StartPath(Path path, bool pushToFront = false)
    {
        ...
        if (path.PipelineState != PathState.Created)
            throw new Exception("The path has an invalid state. Expected " + PathState.Created + " found " +
                                path.PipelineState + "\n" +
        ...
        if (!Application.isPlaying) BlockUntilCalculated(path);
    }
    ...
    ///     This method will search through all graphs and query them for the closest node to this position, and then it will
    ///     return the closest one of those.
    ...
    ///     The NNConstraint can be used to specify constraints on which nodes can be chosen such as only picking walkable
    ///     nodes.
    ///     <code>
    ...
    public NNInfo GetNearest(Vector3 position, NNConstraint constraint, GraphNode hint)
    {
        ...
        if (graphs != null)
            for (var i = 0; i < graphs.Length; i++)
            {
                ...
                if (graph == null || !constraint.SuitableGraph(i, graph)) continue;
                ...
                var dist = (nnInfo.clampedPosition - position).magnitude;
        ...
        // No matches found
        if (nearestGraph == -1) return new NNInfo();
