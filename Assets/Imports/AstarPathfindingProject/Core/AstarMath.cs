...
namespace Pathfinding
{
	...
	public static class VectorMath
    {
        ...
        {
            var s = (b.x - a.x) * (long)(p.z - a.z) - (p.x - a.x) * (long)(b.z - a.z);
        ...
        {
            var v = (b.x - a.x) * (long)(c.z - a.z) - (c.x - a.x) * (long)(b.z - a.z);
        ...
        public static bool SegmentsIntersectXZ(Vector3 start1, Vector3 end1, Vector3 start2, Vector3 end2)
        {
            ...
            if (u < 0F || u > 1F || u2 < 0F || u2 > 1F) return false;
        ...
        public static bool RaySegmentIntersectXZ(Int3 start1, Int3 end1, Int3 start2, Int3 end2)
        {
            ...
            if (!((nom < 0) ^ (den < 0))) return false;

            //factor2 < 0
            if (!((nom2 < 0) ^ (den < 0))) return false;

            if ((den >= 0 && nom2 > den) || (den < 0 && nom2 <= den)) return false;

            return true;
        }

        /// <summary>
        ///     Returns the intersection factors for line 1 and line 2. The intersection factors is a distance along the line start
        ///     - end where the other line intersects it.\n
        ...
        ///     Lines are treated as infinite.\n
        ///     The second "line" is treated as a ray, meaning only matches on start2 or forwards towards end2 (and beyond) will be
        ///     returned
        ...
        public static float LineRayIntersectionFactorXZ(Int3 start1, Int3 end1, Int3 start2, Int3 end2)
        {
            ...
            if ((float)nom2 / den < 0) return float.NaN;
        ...
        /// <summary>
        ///     Returns the intersection point between the two lines. Lines are treated as infinite. start1 is returned if the
        ///     lines are parallel
        /// </summary>
	...
	public static class AstarMath
    {
        ...
        /// <summary>
        ///     Returns a nicely formatted string for the number of bytes (KiB, MiB, GiB etc). Uses decimal names (KB, Mb -
        ///     1000) but calculates using binary values (KiB, MiB - 1024)
        /// </summary>
        public static string FormatBytesBinary(int bytes)
        {
            ...
            if (bytes < 1024) return bytes * sign + " bytes";

            if (bytes < 1024 * 1024) return (bytes / 1024D * sign).ToString("0.0") + " KiB";

            if (bytes < 1024 * 1024 * 1024) return (bytes / (1024D * 1024D) * sign).ToString("0.0") + " MiB";
        ...
        public static Color HSVToRGB(float h, float s, float v)
        {
            ...
            var Chroma = s * v;
            var Hdash = h / 60.0f;
            var X = Chroma * (1.0f - Math.Abs(Hdash % 2.0f - 1.0f));
	...
	public static class Polygon
    {
        ...
        private static readonly Dictionary<Int3, int> cached_Int3_int_dict = new();
        ...
        ///     Returns: The interpolated y coordinate unless the triangle is degenerate in which case a DivisionByZeroException
        ///     will be thrown
            ...
            out int[] outTriangles)
        {
            ...
            for (var i = 0; i < outTriangles.Length; i++) outTriangles[i] = compressedPointers[triangles[i]];
        ...
        ///     indicating if the contour is a cycle or a chain (see image).
        /// </param>
        public static void TraceContours(Dictionary<int, int> outline, HashSet<int> hasInEdge,
            Action<List<int>, bool> results)
        {
            ...
            for (var k = 0; k <= 1; k++)
            {
                ...
                for (var i = 0; i < outlineKeys.Count; i++)
                {
                    ...
                    if (!cycles && hasInEdge.Contains(startIndex)) continue;
                    ...
                    if (obstacleVertices.Count > 1) results(obstacleVertices, cycles);
                }
            }
