...
namespace Pathfinding
{
    ...
    public abstract class Path : IPathInternals
    {
        ...
        ///     This is usually sent to the Seeker component which post processes the path and then calls a callback to the script
        ///     which requested the path
        /// </summary>
        ...
        ///     \bug This may currently be set to Complete before the path has actually been fully calculated. In particular the
        ///     vectorPath and path lists may not have been fully constructed.
        ///     This can lead to race conditions when using multithreading. Try to avoid using this method to check for if the path
        ///     is calculated right now, use <see cref="IsDone" /> instead.
        /// </summary>
        ...
        ///     Note: Setting this to null, or trying to assign an array which does not have a length of 32, will make all tag
        ///     penalties be treated as if they are zero.
        ///     Note: If you are using a Seeker. The Seeker will set this value to what is set in the inspector field when you call
        ///     seeker.StartPath.
        ...
        ///     Cache this rather than call this function every time since it will calculate the length every time, not just return
        ///     a cached value.
        ///     Returns: Total length of <see cref="vectorPath" />, if <see cref="vectorPath" /> is null positive infinity is
        ///     returned.
        /// </summary>
        ...
        ///     \throws System.InvalidOperationException if the path is not started. Send the path to Seeker.StartPath or
        ///     AstarPath.StartPath before calling this function.
        ...
        internal uint CalculateHScore(GraphNode node)
        {
            ...
            switch (heuristic)
            {
                ...
                case Heuristic.Manhattan:
                    ...
                    h = (uint)((Math.Abs(hTarget.x - p2.x) + Math.Abs(hTarget.y - p2.y) + Math.Abs(hTarget.z - p2.z)) *
                               heuristicScale);
                    return h;
        ...
        ///     The cost of moving between the nodes. Return this value if there is no meaningful special
        ///     cost to return.
        /// </param>
        ...
        ///     See: \reflink{Seeker.IsDone} which also takes into account if the %path %callback has been called and had modifiers
        ///     applied.
        /// </summary>
        public bool IsDone()
        {
        ...
        ///     Note: If AstarPath.logPathResults is PathLog.None and this is a standalone player, nothing will be logged as an
        ///     optimization.
        ///     Deprecated: Use <see cref="FailWithError" /> instead
        /// </summary>
        [Obsolete("Use FailWithError instead")]
        ...
        private readonly List<object> claimed = new();
        ...
        public void Claim(object o)
        {
            ...
            for (var i = 0; i < claimed.Count; i++)
                ...
                if (ReferenceEquals(claimed[i], o))
                    throw new ArgumentException("You have already claimed the path with that object (" + o +
        ...
        ///     When the claim count reaches zero, the path will be pooled, all variables will be cleared and the path will be put
        ///     in a pool to be used again.
        ...
        ///     but the path will not be pooled if the claim count reches zero unless a Release call (not silent) has been made
        ///     earlier.
        ///     This is used by the internal pathfinding components such as Seeker and AstarPath so that they will not cause paths
        ///     to be pooled.
        ///     This enables users to skip the claim/release calls if they want without the path being pooled by the Seeker or
        ///     AstarPath and
        ...
        public void Release(object o, bool silent = false)
        {
...
#endif
                    if (!silent) releasedNotSilent = true;

                    if (claimed.Count == 0 && releasedNotSilent) PathPool.Pool(this);
                    return;
                }
            ...
            throw new ArgumentException("You are releasing a path which has not been claimed with this object (" + o +
                                        "). " +
        ...
        ///     This will build an array (<see cref="path)" /> of the nodes this path will pass through and also set the
        ///     <see cref="vectorPath" /> array to the <see cref="path" /> arrays positions.
        ///     Assumes the <see cref="vectorPath" /> and <see cref="path" /> are empty and not null (which will be the case for a
        ///     correctly initialized path).
        /// </summary>
        protected virtual void Trace(PathNode from)
        {
            ...
            for (var i = 0; i < count; i++) vectorPath.Add((Vector3)path[i].position);
        }
        ...
        {
            if (logMode == PathLog.None || (!error && logMode == PathLog.OnlyErrors)) return "";
        ...
        protected void PrepareBase(PathHandler pathHandler)
        {
            ...
            if (pathHandler.PathID > pathID) pathHandler.ClearPathIDs();
