...
namespace Pathfinding
{
	...
	public interface IAstarAI
    {
	    ...
	    ///     Note: The <see cref="Pathfinding.AILerp" /> script doesn't really have any use of knowing the radius or the height
	    ///     of the character, so this property will always return 0 in that script.
	    /// </summary>
	    ...
	    ///     This is visualized in the scene view as a yellow cylinder around the character.
	    ///     This value is currently only used if an RVOController is attached to the same GameObject, otherwise it is only used
	    ///     for drawing nice gizmos in the scene view.
	    ///     However since the height value is used for some things, the radius field is always visible for consistency and
	    ///     easier visualization of the character.
        ...
        ///     Note: This is the distance to the end of the path, which may or may not be at the <see cref="destination" />. If
        ///     the character cannot reach the destination it will try to move as close as possible to it.
        ///     Warning: Since path requests are asynchronous, there is a small delay between a path request being sent and this
        ///     value being updated with the new calculated path.
        ...
        ///     For the AIPath/RichAI scripts, this is when the character is within <see cref="AIPath.endReachedDistance" /> world
        ///     units from the <see cref="destination" />.
        ///     For the AILerp script it is when the character is at the destination (±a very small margin).
        ///     This value will be updated immediately when the <see cref="destination" /> is changed (in contrast to
        ///     <see cref="reachedEndOfPath)" />, however since path requests are asynchronous
        ///     it will use an approximation until it sees the real path result. What this property does is to check the distance
        ///     to the end of the current path, and add to that the distance
        ///     from the end of the path to the <see cref="destination" /> (i.e. is assumes it is possible to move in a straight
        ///     line between the end of the current path to the destination) and then checks if that total
        ///     distance is less than <see cref="endReachedDistance" />. This property is therefore only a best effort, but it will
        ///     work well for almost all use cases.
        ///     Furthermore it will not report that the destination is reached if the destination is above the head of the
        ///     character or more than half the <see cref="height" /> of the character below its feet
        ///     (so if you have a multilevel building, it is important that you configure the <see cref="height" /> of the
        ///     character correctly).
        ///     The cases which could be problematic are if an agent is standing next to a very thin wall and the destination
        ///     suddenly changes to the other side of that thin wall.
        ///     During the time that it takes for the path to be calculated the agent may see itself as alredy having reached the
        ///     destination because the destination only moved a very small distance (the wall was thin),
        ///     even though it may actually be quite a long way around the wall to the other side.
        ///     In contrast to <see cref="reachedEndOfPath" />, this property is immediately updated when the
        ///     <see cref="destination" /> is changed.
        ///     <code>
        /// IEnumerator Start () {
        ...
        ///     Note that setting the <see cref="destination" /> does not immediately update the path, nor is there any guarantee
        ///     that the
        ...
        ///     It is very hard to provide a method for detecting if the AI has reached the <see cref="destination" /> that works
        ///     across all different games
        ///     because the destination may not even lie on the navmesh and how that is handled differs from game to game (see also
        ///     the code snippet in the docs for <see cref="destination)" />.
        ///     See: <see cref="remainingDistance" />
        ///     See: <see cref="reachedDestination" />
        /// </summary>
        ...
        ///     then you could either use <see cref="reachedDestination" /> (recommended) or check both <see cref="pathPending" />
        ///     and <see cref="reachedEndOfPath" />.
        ...
        ///     Setting this to false does not stop any active path requests from being calculated or stop it from continuing to
        ///     follow the current path.
        ///     Note that this only disables automatic path recalculations. If you call the <see cref="SearchPath()" /> method a
        ///     path will still be calculated.
        ...
        ///     If you want the agent to stand still, but still react to local avoidance and use gravity: use
        ///     <see cref="isStopped" /> instead.
        ...
        ///     The agent will still react to local avoidance and gravity (if applicable), but it will not try to move in any
        ...
        ///     If this property is set to true while the agent is traversing an off-mesh link (RichAI script only), then the agent
        ///     will
        ///     continue traversing the link and stop once it has completed it.
        ///     Note: This is not the same as the <see cref="canMove" /> setting which some movement scripts have. The
        ///     <see cref="canMove" /> setting
        ///     disables movement calculations completely (which among other things makes it not be affected by local avoidance or
        ///     gravity).
        ///     For the AILerp movement script which doesn't use gravity or local avoidance anyway changing this property is very
        ///     similar to
        ///     changing <see cref="canMove" />.
        ///     The <see cref="steeringTarget" /> property will continue to indicate the point which the agent would move towards
        ///     if it would not be stopped.
        /// </summary>
        ...
        ///     This is called both for automatic path recalculations (see <see cref="canSearch)" /> and manual ones (see
        ///     <see cref="SearchPath)" />.
        ///     See: Take a look at the <see cref="Pathfinding.AIDestinationSetter" /> source code for an example of how it can be
        ///     used.
        /// </summary>
        ...
        /// </summary>
        /// <param name="buffer">
        ///     The buffer will be cleared and replaced with the path. The first point is the current position of
        ///     the agent.
        /// </param>
        /// <param name="stale">
        ///     May be true if the path is invalid in some way. For example if the agent has no path or (for the
        ///     RichAI script only) if the agent has detected that some nodes in the path have been destroyed.
        /// </param>
        ...
        ///     If there is an ongoing path calculation, it will be canceled, so make sure you leave time for the paths to get
        ///     calculated before calling this function again.
        ///     A canceled path will show up in the log with the message "Canceled by script" (see
        ///     <see cref="Seeker.CancelCurrentPathRequest())" />.
        ...
        ///     This means the AI may not actually start to follow the path until in a few frames when the path has been
        ///     calculated.
        ...
        ///     Note that if you calculate the path using seeker.StartPath then this script will already pick it up because it is
        ///     listening for
        ...
        ///     Note than unless you have also disabled <see cref="canSearch" /> then the agent will soon recalculate its path and
        ///     start moving again.
        ...
        ///     This will trigger a path recalculation (if clearPath is true, which is the default) so if you want to teleport the
        ///     agent and change its <see cref="destination" />
        ...
        ///     applied the next time the agent runs its movement calculations (which is usually later this frame or the next
        ///     frame).
