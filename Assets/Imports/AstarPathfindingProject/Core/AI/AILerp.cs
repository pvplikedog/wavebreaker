...
namespace Pathfinding
{
	...
	///     This movement script will follow the path exactly, it uses linear interpolation to move between the waypoints in
	///     the path.
	...
	///     If you are aiming for movement where the unit follows the path exactly and move only along the graph connections on
	///     a grid/point graph.
	///     I recommend that you adjust the StartEndModifier on the Seeker component: set the 'Start Point Snapping' field to
	///     'NodeConnection' and the 'End Point Snapping' field to 'SnapToNode'.
	///     [Open online documentation to see images]
	///     [Open online documentation to see images]
	///     \subsection rec-smooth Recommended setup for smooth movement
	///     If you on the other hand want smoother movement I recommend setting 'Start Point Snapping' and 'End Point Snapping'
	///     to 'ClosestOnNode' and to add the Simple Smooth Modifier to the GameObject as well.
	///     Alternatively you can use the <see cref="Pathfinding.FunnelModifier Funnel" /> which works better on navmesh/recast
	///     graphs or the <see cref="Pathfinding.RaycastModifier" />.
	///     You should not combine the Simple Smooth Modifier or the Funnel Modifier with the NodeConnection snapping mode.
	///     This may lead to very odd behavior.
	///     [Open online documentation to see images]
	///     [Open online documentation to see images]
    ...
    public class AILerp : VersionedMonoBehaviour, IAstarAI
    {
        ...
        ///     Using the YAxisForward option will also allow the agent to assume that the movement will happen in the 2D (XY)
        ///     plane instead of the XZ plane
        ///     if it does not know. This is important only for the point graph which does not have a well defined up direction.
        ///     The other built-in graphs (e.g the grid graph)
        ///     will all tell the agent which movement plane it is supposed to use.
        ///     [Open online documentation to see images]
        /// </summary>
        [FormerlySerializedAs("rotationIn2D")] public OrientationMode orientation = OrientationMode.ZAxisForward;
        ...
        [FormerlySerializedAs("target")] [SerializeField] [HideInInspector]
        private Transform targetCompatibility;

        [SerializeField] [HideInInspector] [FormerlySerializedAs("repathRate")]
        private float repathRateCompatibility = float.NaN;

        [SerializeField] [HideInInspector] [FormerlySerializedAs("canSearch")]
        ...
        ///     If false then all movement calculations will happen as usual, but the object that this component is attached to
        ///     will not move
        ...
        /// </summary>
        [NonSerialized] public bool updatePosition = true;
        ...
        /// </summary>
        [NonSerialized] public bool updateRotation = true;
        ...
        ///     It can be a point on the ground where the player has clicked in an RTS for example, or it can be the player object
        ///     in a zombie game.
        ///     Deprecated: In 4.0 this will automatically add a <see cref="Pathfinding.AIDestinationSetter" /> component and set
        ///     the target on that component.
        ///     Try instead to use the <see cref="destination" /> property which does not require a transform to be created as the
        ///     target or use
        ...
        protected virtual bool shouldRecalculatePath => canSearchAgain && autoRepath.ShouldRecalculatePath(this);
        ...
        public virtual void SearchPath()
        {
            ...
            SetPath(ABPath.Construct(currentPosition, destination));
        }
        ...
        protected virtual void OnPathComplete(Path _p)
        {
            ...
            if (interpolatePathSwitches) ConfigurePathSwitchInterpolation();
            ...
            if (path.vectorPath != null && path.vectorPath.Count == 1) path.vectorPath.Insert(0, GetFeetPosition());
        ...
        protected virtual Vector3 CalculateNextPosition(out Vector3 direction, float deltaTime)
        {
            ...
            if (interpolatePathSwitches && alpha < 1f)
            {
                ...
                var positionAlongPreviousPath = previousMovementOrigin +
...
#pragma warning restore 618
        ...
        public Vector3 velocity => Time.deltaTime > 0.00001f
