...
namespace Pathfinding
{
    ...
    public struct Connection
    {
        ...
        ///     Used for mesh nodes.
        ///     A value of 0 corresponds to using the side for vertex 0 and vertex 1 on the node. 1 corresponds to vertex 1 and 2,
        ///     etc.
    ...
    public abstract class GraphNode
    {
        ...
        /// <summary>
        ///     Internal unique index. Also stores some bitpacked values such as <see cref="TemporaryFlag1" /> and
        ///     <see cref="TemporaryFlag2" />.
        /// </summary>
        ...
        /// </summary>
        public NavGraph Graph => Destroyed ? null : AstarData.GetGraph(this);
        ...
        {
            get => (nodeIndex & TemporaryFlag1Mask) != 0;
        ...
        {
            get => (nodeIndex & TemporaryFlag2Mask) != 0;
        ...
        ///     The graph is responsible for calling this method on nodes when they are destroyed, including when the whole graph
        ///     is destoyed.
        ///     Otherwise memory leaks might present themselves.
        ///     Once called the <see cref="Destroyed" /> property will return true and subsequent calls to this method will not do
        ///     anything.
        ...
        public void Destroy()
        {
            ...
            if (AstarPath.active != null) AstarPath.active.DestroyNode(this);
        ...
        ///     You must call this method if you change the connectivity or walkability of the node without going through the high
        ///     level methods
        ///     such as the <see cref="Walkable" /> property or the <see cref="AddConnection" /> method. For example if your
        ///     manually change the <see cref="Pathfinding.MeshNode.connections" /> array you need to call this method.
        /// </summary>
        ...
        ///     Deprecated: This method is deprecated because it never did anything, you can safely remove any calls to this
        ///     method.
        /// </summary>
        [Obsolete(
        ...
        ///     Returns: True if the call was deemed successful. False if some unknown case was encountered and no portal could be
        ///     added.
        ///     If both calls to node1.GetPortal (node2,...) and node2.GetPortal (node1,...) return false, the funnel modifier will
        ///     fall back to adding to the path
        ...
        /// </summary>
        /// <param name="other">
        ///     The node which is on the other side of the portal (strictly speaking it does not actually have to
        ///     be on the other side of the portal though).
        /// </param>
        ...
        ///     If this is true, the call was made on a node with the other node as the node before this one in the path.
        ///     In this case you may choose to do nothing since a similar call will be made to the other node with this node
        ///     referenced as other (but then with backwards = true).
        ...
        ///     The graph is divided into clusters of small hierarchical nodes in which there is a path from every node to every
        ///     other node.
        ///     This structure is used to speed up connected component calculations which is used to quickly determine if a node is
    ...
    public abstract class MeshNode : GraphNode
    {
        ...
        ///     This is usually very similar to <see cref="ClosestPointOnNode" /> but when the node is in a slope this can be
        ///     significantly different.
        ///     [Open online documentation to see images]
        ///     When the blue point in the above image is used as an argument this method call will return the green point while
        ///     the <see cref="ClosestPointOnNode" /> method will return the red point.
        /// </summary>
        ...
        /// <param name="cost">
        ///     Cost of traversing the connection. A cost of 1000 corresponds approximately to the cost of moving 1
        ///     world unit.
        /// </param>
        ...
        public void AddConnection(GraphNode node, uint cost, int shapeEdge)
        {
            ...
            for (var i = 0; i < connLength; i++) newconns[i] = connections[i];

            newconns[connLength] = new Connection(node, cost, (byte)shapeEdge);

            if (connections != null) ArrayPool<Connection>.Release(ref connections, true);
        ...
        public override void RemoveConnection(GraphNode node)
        {
            ...
            for (var i = 0; i < connections.Length; i++)
                if (connections[i].node == node)
                {
                    ...
                    var newconns = ArrayPool<Connection>.ClaimWithExactLength(connLength - 1);
                    for (var j = 0; j < i; j++) newconns[j] = connections[j];
                    for (var j = i + 1; j < connLength; j++) newconns[j - 1] = connections[j];
