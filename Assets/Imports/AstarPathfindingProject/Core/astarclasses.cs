...
namespace Pathfinding
{
    ...
    public class AstarColor
    {
        ...
        ///     If the user has set a color for the area, it is used, but otherwise the color is calculated using
        ///     AstarMath.IntToColor
        ...
        ///     If the user has set a color for the tag, it is used, but otherwise the color is calculated using
        ///     AstarMath.IntToColor
    ...
    ///     Some members will also be initialized even if nothing was hit, see the individual member descriptions for more
    ///     info.
    ...
    public struct GraphHitInfo
    {
	    ...
	    ///     If the linecast did not hit anything then this will be set to the last node along the line's path (the one which
	    ///     contains the endpoint).
	    ///     For layered grid graphs the linecast will return true (i.e: no free line of sight) if when walking the graph we
	    ///     ended up at X,Z coordinate for the end node
	    ///     even if end node was on a different level (e.g the floor below or above in a building). In this case no node edge
	    ///     was really hit so this field will still be null.
	    /// </summary>
	    public GraphNode node;

	    /// <summary>
	    ///     Where the tangent starts. <see cref="tangentOrigin" /> and <see cref="tangent" /> together actually describes the
	    ///     edge which was hit.
	    ///     [Open online documentation to see images]
	    /// </summary>
    ...
    /// <summary>
    ///     Nearest node constraint. Constrains which nodes will be returned by the <see cref="AstarPath.GetNearest" />
    ///     function
    /// </summary>
    public class NNConstraint
    {
	    /// <summary>
	    ///     Area ID to constrain to. Will not affect anything if less than 0 (zero) or if <see cref="constrainArea" /> is
	    ///     false
	    /// </summary>
	    public int area = -1;

	    /// <summary>
	    ///     Only treat nodes in the area <see cref="area" /> as suitable. Does not affect anything if <see cref="area" />
	    ///     is less than 0 (zero)
	    /// </summary>
	    ...
	    ///     If there are no suitable nodes within the distance limit then the search will terminate with a null node as a
	    ///     result.
	    ...
	    ///     This can be important on sloped surfaces. See the image below in which the closest point for each blue point is
	    ///     queried for:
	    ///     [Open online documentation to see images]
	    ///     The navmesh/recast graphs also contain a global option for this:
	    ///     <see cref="Pathfinding.NavmeshBase.nearestSearchOnlyXZ" />.
	    /// </summary>
	    public bool distanceXZ;

	    /// <summary>
	    ///     Graphs treated as valid to search on.
	    ///     This is a bitmask meaning that bit 0 specifies whether or not the first graph in the graphs list should be able to
	    ...
	    /// </code>
	    ///     Note: This does only affect which nodes are returned from a <see cref="AstarPath.GetNearest" /> call, if a valid
	    ///     graph is connected to an invalid graph using a node link then it might be searched anyway.
	    ...
	    /// </summary>
	    public static NNConstraint Default => new();
    ...
    ///     A PathNNConstraint can be assigned to the Path.nnConstraint field, the path will first search for the start node,
    ///     then it will call <see cref="SetStart" /> and proceed with searching for the end node (nodes in the case of a
    ///     MultiTargetPath).\n
    ...
    public class PathNNConstraint : NNConstraint
    {
        ...
        /// <summary>
        ///     Called after the start node has been found. This is used to get different search logic for the start and end
        ///     nodes in a path
        /// </summary>
    ...
    public interface IUpdatableGraph
    {
	    ...
	    ///     -# Call o.WillUpdateNode on the GUO for every node it will update, it is important that this is called BEFORE any
	    ...
	    ///     -# Update connectivity info if appropriate (GridGraphs updates connectivity, but most other graphs don't since then
	    ///     the connectivity cannot be recovered later).
	    /// </summary>
    ...
    public class GraphUpdateObject
    {
        ...
        /// <summary>
        ///     NNConstraint to use.
        ///     The Pathfinding.NNConstraint.SuitableGraph function will be called on the NNConstraint to enable filtering of which
        ///     graphs to update.\n
        ///     Note: As the Pathfinding.NNConstraint.SuitableGraph function is A* Pathfinding Project Pro only, this variable
        ///     doesn't really affect anything in the free version.
        /// </summary>
        ...
        ///     The images below shows two overlapping graph update objects, the right one happened to be applied before the left
        ///     one. They both have updatePhysics = true and are
        ...
        ///     [Open online documentation to see images]
        ///     This second image shows when resetPenaltyOnPhysics is set to true. The first GUO is applied correctly, but then the
        ///     second one (the left one) is applied
        ///     and during its updating, it resets the penalties first and then adds penalty to the nodes. The result is that the
        ///     penalties from both GUOs are not added together.
        ///     The green patch in at the border is there because physics recalculation (recalculation of the position of the node,
        ///     checking for obstacles etc.) affects a slightly larger
        ///     area than the original GUO bounds because of the Grid Graph -> Collision Testing -> Diameter setting (it is
        ///     enlarged by that value). So some extra nodes have their penalties reset.
        ///     [Open online documentation to see images]
        /// </summary>
        ...
        ///     The first image shows the graph when no GUO has been applied. The blue box is not identified as an obstacle by the
        ///     graph, the reason
        ///     there are unwalkable nodes around it is because there is a height difference (nodes are placed on top of the box)
        ///     so erosion will be applied (an erosion value of 2 is used in this graph).
        ///     The orange box is identified as an obstacle, so the area of unwalkable nodes around it is a bit larger since both
        ///     erosion and collision has made
        ///     nodes unwalkable.\n
        ///     The GUO used simply sets walkability to true, i.e making all nodes walkable.
        ///     [Open online documentation to see images]
        ///     When updateErosion=True, the reason the blue box still has unwalkable nodes around it is because there is still a
        ///     height difference
        ///     so erosion will still be applied. The orange box on the other hand has no height difference and all nodes are set
        ///     to walkable.\n
        ///     \n
        ...
        ///     When updating a grid graph and this is true, the nodes' position and walkability will be updated using physics
        ///     checks
        ///     with settings from "Collision Testing" and "Height Testing".
        ///     When updating a PointGraph, setting this to true will make it re-evaluate all connections in the graph which passes
        ///     through the <see cref="bounds" />.
        ...
        ///     For example when only updating penalty values it can save processing power when setting this to false. Especially
        ///     on large graphs.
        ///     Note: If you set this to false, even though it does change e.g walkability, it can lead to paths returning that
        ///     they failed even though there is a path,
        ///     or the try to search the whole graph for a path even though there is none, and will in the processes use wast
        ///     amounts of processing power.
        ///     If using the basic GraphUpdateObject (not a derived class), a quick way to check if it is going to need a flood
        ///     fill is to check if <see cref="modifyWalkability" /> is true or <see cref="updatePhysics" /> is true.
        ...
        ///     Note: Not all data is saved. The saved data includes: penalties, walkability, tags, area, position and for grid
        ///     graphs (not layered) it also includes connection data.
        ///     This method modifies the graph. So it must be called inside while it is safe to modify the graph, for example
        ///     inside a work item as shown in the example below.
        ...
        public virtual void Apply(GraphNode node)
        {
            if (shape == null || shape.Contains(node))
            {
                ...
                if (modifyWalkability) node.Walkable = setWalkability;
    ...
    public struct GraphMask
    {
        ...
        public static GraphMask everything => new(-1);
    ...
    ///     The different heuristics have roughly the same performance except not using any heuristic at all (
    ///     <see cref="None)" />
    ...
    ///     they only do it in this case because of very small rounding errors. Usually they behave almost identically on
    ///     8-connected grids.
    ///     [Open online documentation to see images]
    ///     Generally for a 4-connected grid graph the Manhattan option should be used as it is the true distance on a
    ///     4-connected grid.
    ///     For an 8-connected grid graph the Diagonal Manhattan option is the mathematically most correct option, however the
    ///     Euclidean option
    ...
    public enum Heuristic
    {
        ...
        ///     Note: This option is currently hard coded for the XZ plane. It will be equivalent to Manhattan distance if you try
        ///     to use it in the XY plane (i.e for a 2D game).
        /// </summary>
        DiagonalManhattan,
        ...
        ///     This is usually significantly slower compared to using a heuristic, which is why the A* algorithm is usually
        ///     preferred over Dijkstra's algorithm.
        ///     You may have to use this if you have a very non-standard graph. For example a world with a
        ///     <a href="https://en.wikipedia.org/wiki/Wraparound_(video_games)">wraparound playfield</a> (think Civilization or
        ///     Asteroids) and you have custom links
        ///     with a zero cost from one end of the map to the other end. Usually the A* algorithm wouldn't find the wraparound
        ///     links because it wouldn't think to look in that direction.
    ...
    public enum CloseToDestinationMode
    {
	    ...
	    ///     movement scripts) units from the destination
	    /// </summary>
	    Stop,
