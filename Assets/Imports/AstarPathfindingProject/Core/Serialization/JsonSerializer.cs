...
namespace Pathfinding.Serialization
{
    ...
    public class GraphSerializationContext
    {
        ...
        {
            if (reader.BaseStream.Position <= reader.BaseStream.Length - 4) return reader.ReadInt32();
        ...
        {
            if (reader.BaseStream.Position <= reader.BaseStream.Length - 4) return reader.ReadSingle();
        ...
        {
            var inst = reader.ReadInt32();

            if (inst == int.MaxValue) return null;
            ...
            if (!string.IsNullOrEmpty(guid))
            {
                ...
                for (var i = 0; i < helpers.Length; i++)
                    ...
                    {
                        if (type == typeof(GameObject)) return helpers[i].gameObject;
    ...
    public class AstarSerializer
    {
        ...
        public static readonly Version V3_8_3 = new(3, 8, 3);

        /// <summary>Cached version object for 3.9.0</summary>
        public static readonly Version V3_9_0 = new(3, 9, 0);

        /// <summary>Cached version object for 4.1.0</summary>
        public static readonly Version V4_1_0 = new(4, 1, 0);
        ...
        /// </summary>
        private readonly GameObject contextRoot;

        private readonly AstarData data;
        ...
        private readonly SerializeSettings settings;
        ...
        {
            if (graphs == null) throw new Exception("No call to SerializeGraphs has been done");
        ...
        private static int GetMaxNodeIndexInAllGraphs(NavGraph[] graphs)
        {
            ...
            for (var i = 0; i < graphs.Length; i++)
            {
                ...
                graphs[i].GetNodes(node =>
                {
                    ...
                    if (node.NodeIndex == -1) Debug.LogError("Graph contains destroyed nodes. This is a bug.");
                });
            }
        ...
        private static byte[] SerializeNodeIndices(NavGraph[] graphs)
        {
            ...
            var maxNodeIndex = GetMaxNodeIndexInAllGraphs(graphs);
        ...
        private NavGraph DeserializeGraph(int zipIndex, int graphIndex, Type[] availableGraphTypes)
        {
            ...
            if (Equals(graphType, null)) return null;
        ...
        private GraphNode[] DeserializeNodeReferenceMap()
        {
            ...
            var reader = GetBinaryReader(entry);
            var maxNodeIndex = reader.ReadInt32();
        ...
        {
            var anyDeserialized = false;

            // Loop through all graphs and deserialize the extra info
            // if there is any such info in the zip file
            for (var i = 0; i < graphs.Length; i++) anyDeserialized |= DeserializeExtraInfo(graphs[i]);

            if (!anyDeserialized) return;

            // Sanity check
            // Make sure the graphs don't contain destroyed nodes
            if (AnyDestroyedNodesInGraphs()) Debug.LogError("Graph contains destroyed nodes. This is a bug.");

            // Deserialize map from old node indices to new nodes
            var int2Node = DeserializeNodeReferenceMap();

            // Deserialize node references
            for (var i = 0; i < graphs.Length; i++) DeserializeNodeReferences(graphs[i], int2Node);
        ...
        ///     For future compatibility this method does not assume that the graphEditors array matches the <see cref="graphs" />
        ///     array in order and/or count.
        ...
        ///     Note: This method is only used for compatibility, newer versions store everything in the
        ///     graph.serializedEditorSettings field which is already serialized.
        /// </summary>
        public void DeserializeEditorSettingsCompatibility()
        {
            for (var i = 0; i < graphs.Length; i++)
            {
                ...
                var entry = GetEntry("graph" + zipIndex + "_editor" + jsonExt);
        ...
        private GraphMeta DeserializeBinaryMeta(ZipEntry entry)
        {
            ...
            if (reader.ReadString() != "A*") throw new Exception("Invalid magic number in saved data");
            ...
            for (var i = 0; i < count; i++) meta.guids.Add(reader.ReadString());

            meta.typeNames = new List<string>();
            count = reader.ReadInt32();
            for (var i = 0; i < count; i++) meta.typeNames.Add(reader.ReadString());
            reader.Close();
    ...
    public class GraphMeta
    {
        ...
        public Type GetGraphType(int index, Type[] availableGraphTypes)
        {
            ...
            for (var j = 0; j < availableGraphTypes.Length; j++)
                if (availableGraphTypes[j].FullName == typeNames[index])
