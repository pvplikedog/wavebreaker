...
namespace Pathfinding
{
    ...
    public class GraphUpdateScene : GraphModifier
    {
        ...
        ///     Usually you need quite large values, at least 1000-10000. A higher penalty means that agents will try to avoid
        ///     those nodes more.
        ...
        /// <summary>
        ///     If true, then all affected nodes will be made walkable or unwalkable according to
        ///     <see cref="setWalkability" />
        /// </summary>
        ...
        ///     For Point Graphs, this will recalculate all connections which passes through the bounds of the resulting Graph
        ///     Update Object
        ...
        [HideInInspector] public bool legacyMode;
        ...
        /// </summary>
        [SerializeField] [FormerlySerializedAs("useWorldSpace")]
        ...
        public void Start()
        {
            ...
            if (!firstApplied && applyOnStart) Apply();
        }
        ...
        {
            var c = selected
                ? new Color(227 / 255f, 61 / 255f, 22 / 255f, 1.0f)
                : new Color(227 / 255f, 61 / 255f, 22 / 255f, 0.9f);
            ...
            if (selected || !convex)
                for (var i = 0; i < points.Length; i++)
                    Gizmos.DrawLine(matrix.MultiplyPoint3x4(points[i]),
                        matrix.MultiplyPoint3x4(points[(i + 1) % points.Length]));
            ...
            if (convex)
            {
                ...
                for (var i = 0; i < convexPoints.Length; i++)
                    Gizmos.DrawLine(matrix.MultiplyPoint3x4(convexPoints[i]),
                        matrix.MultiplyPoint3x4(convexPoints[(i + 1) % convexPoints.Length]));
            }
        ...
        ///     <see cref="setTag" /> will be stored in a private variable, and the new value will be 0. When calling this function
        ///     again, the saved
        ///     value will be the new value.
        ///     Calling this function an even number of times without changing any settings in between will be identical to no
        ///     change in settings.
        /// </summary>
        ...
        ///     Deprecated: World space can no longer be used as it does not work well with rotated graphs. Use
        ///     transform.InverseTransformPoint to transform points to local space.
        /// </summary>
        [Obsolete(
        ...
        public Bounds GetBounds()
        {
            ...
            return GraphUpdateShape.GetBounds(convex ? convexPoints : points,
                legacyMode && legacyUseWorldSpace ? Matrix4x4.identity : transform.localToWorldMatrix, minBoundsHeight);
        }
        ...
        public void Apply()
        {
            ...
            if (points == null || points.Length == 0)
            {
                ...
                else
                {
                    ...
                    {
                        Debug.LogError(
                            "Cannot apply GraphUpdateScene, no points defined and no renderer or collider attached",
                            this);
                        return;
                    }
        ...
        public void DisableLegacyMode()
        {
            if (legacyMode)
            {
                ...
                if (legacyUseWorldSpace)
                {
                    ...
                    for (var i = 0; i < points.Length; i++) points[i] = transform.InverseTransformPoint(points[i]);
                    RecalcConvex();
                }
            }
        }
