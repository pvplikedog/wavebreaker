...
namespace Pathfinding
{
    ...
    public class AutoRepathPolicy
    {
        ...
        public enum Mode
        {
	        ...
	        ///     Paths can be recalculated manually by for example calling \reflink{IAstarAI.SearchPath} or
	        ///     \reflink{IAstarAI.SetPath}.
            ...
            /// <summary>
            ///     Recalculate the path at least every \reflink{maximumInterval} seconds but more often if the destination moves a
            ///     lot.
            ///     This mode is recommended since it allows the agent to quickly respond to new destinations without using up a lot of
            ///     CPU power to calculate paths
            ///     when it doesn't have to.
            ///     More precisely:\n
            ///     Let C be a circle centered at the destination for the last calculated path with a radius equal to the distance to
            ///     that point divided by \reflink{sensitivity}.\n
            ///     If the new destination is outside that circle the path will be immediately recalculated.\n
            ///     Otherwise let F be the 1 - (distance from the circle's center to the new destination divided by the circle's
            ///     radius).\n
            ///     So F will be 1 if the new destination is the same as the old one and 0 if it is at the circle's edge.\n
            ///     Recalculate the path if the time since the last path recalculation is greater than \reflink{maximumInterval}
            ///     multiplied by F.\n
        ...
        public virtual bool ShouldRecalculatePath(IAstarAI ai)
        {
            ...
            if (mode == Mode.EveryNSeconds) return timeSinceLast >= interval;

            // cost = change in destination / max(distance to destination, radius)
            var squaredCost = (ai.destination - lastDestination).sqrMagnitude /
                              Mathf.Max((ai.position - lastDestination).sqrMagnitude, ai.radius * ai.radius);
        ...
        public void DrawGizmos(IAstarAI ai)
        {
            ...
            {
                var r = Mathf.Sqrt(Mathf.Max((ai.position - lastDestination).sqrMagnitude, ai.radius * ai.radius) /
