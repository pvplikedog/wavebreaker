...
namespace Pathfinding
{
    ...
    public class PathProcessor
    {
        ...
        private readonly List<int> locks = new();
        ...
        /// </summary>
        private readonly Stack<int> nodeIndexPool = new();
        ...
        private readonly CustomSampler profilingSampler;
#endif
	    ...
	    ///     This is done so other functions can step the thread forward at any time, without having to wait for Unity to update
	    ///     it.
	    ///     See: CalculatePaths
        ...
        internal PathProcessor(AstarPath astar, PathReturnQueue returnQueue, int processors, bool multithreaded)
        {
            ...
            if (processors < 0) throw new ArgumentOutOfRangeException("processors");
            ...
            for (var i = 0; i < processors; i++) pathHandlers[i] = new PathHandler(i, processors);
        ...
        {
            if (!locks.Remove(id)) throw new ArgumentException("This lock has already been released");
        ...
        /// </summary>
        /// <param name="block">
        ///     If true, this call will block until all pathfinding threads are paused.
        ...
        public void InitializeNode(GraphNode node)
        {
            ...
            for (var i = 0; i < pathHandlers.Length; i++) pathHandlers[i].InitializeNode(node);
        ...
        public void DestroyNode(GraphNode node)
        {
            ...
            for (var i = 0; i < pathHandlers.Length; i++) pathHandlers[i].DestroyNode(node);
        ...
        {
#if UNITY_2017_3_OR_NEWER
            Profiler.BeginThreadProfiling("Pathfinding", "Pathfinding thread #" + (pathHandler.threadID + 1));
#endif
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    if (pathHandler.threadID > 0) throw new Exception("Thread Error");
                    ...
                    // Call some callbacks
                    if (OnPathPreSearch != null) OnPathPreSearch(path);
                    ...
                    if (path.CompleteState == PathCompleteState.NotCalculated)
                    {
                        ...
                        while (path.CompleteState == PathCompleteState.NotCalculated)
                        {
                            ...
                            if (queue.IsTerminating) path.FailWithError("AstarPath object destroyed");
                        }

                        path.duration = (DateTime.UtcNow.Ticks - startTicks) * 0.0001F;
                    ...
                    if (OnPathPostSearch != null) OnPathPostSearch(path);
            ...
            while (true)
            {
                ...
                if (p.CompleteState == PathCompleteState.NotCalculated)
                {
                    ...
                    while (p.CompleteState == PathCompleteState.NotCalculated)
                    {
                        ...
                        if (queue.IsTerminating) p.FailWithError("AstarPath object destroyed");
