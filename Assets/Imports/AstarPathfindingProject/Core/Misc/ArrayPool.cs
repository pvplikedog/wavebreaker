...
namespace Pathfinding.Util
{
	...
	public static class ArrayPool<T>
    {
        ...
        {
            if (minimumLength <= 0) return ClaimWithExactLength(0);

            var bucketIndex = 0;
            while (1 << bucketIndex < minimumLength && bucketIndex < 30) bucketIndex++;
            ...
            lock (pool)
            {
                if (pool[bucketIndex] == null) pool[bucketIndex] = new Stack<T[]>();
	    ...
	    /// <summary>
	    ///     Pool an array.
	    ///     If the array was got using the <see cref="ClaimWithExactLength" /> method then the allowNonPowerOfTwo parameter
	    ///     must be set to true.
	    ///     The parameter exists to make sure that non power of two arrays are not pooled unintentionally which could lead to
	    ///     memory leaks.
	    /// </summary>
	    public static void Release(ref T[] array, bool allowNonPowerOfTwo = false)
        {
            ...
            if (array.GetType() != typeof(T[]))
                throw new ArgumentException("Expected array type " + typeof(T[]).Name + " but found " +
            ...
            lock (pool)
            {
                ...
                if (!inPool.Add(array))
                    throw new InvalidOperationException(
                        "You are trying to pool an array twice. Please make sure that you only pool it once.");
#endif
                if (isPowerOfTwo)
                {
                    var bucketIndex = 0;
                    while (1 << bucketIndex < array.Length && bucketIndex < 30) bucketIndex++;
...
#if !ASTAR_OPTIMIZE_POOLING
        private static readonly HashSet<T[]> inPool = new();
#endif
#endif
    }
    ...
    {
	    /// <summary>
	    ///     Identical to ToArray but it uses ArrayPool
	    ///     <T>
	    ...
	    public static T[] ToArrayFromPool<T>(this List<T> list)
        {
            ...
            for (var i = 0; i < arr.Length; i++) arr[i] = list[i];
            return arr;
        }

	    /// <summary>
	    ///     Clear a list faster than List
	    ///     <T>
	    ///         .Clear.
	    ///         It turns out that the List
	    ///         <T>
	    ///             .Clear method will clear all elements in the underlaying array
