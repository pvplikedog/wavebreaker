    ...
    internal class GraphUpdateProcessor
    {
        ...
        public void DisableMultithreading()
        {
            ...
            if (graphUpdateThread != null && graphUpdateThread.IsAlive)
            {
                ...
                if (!graphUpdateThread.Join(5 * 1000)) Debug.LogError("Graph update thread did not exit in 5 seconds");
        ...
        private bool ProcessGraphUpdates(bool force)
        {
            ...
            {
#if !UNITY_WEBGL
                if (!asyncGraphUpdatesComplete.WaitOne(0)) return false;
#endif
            }
            ...
            ProcessPostUpdates();
            if (!ProcessRegularUpdates(force)) return false;
        ...
        private bool ProcessRegularUpdates(bool force)
        {
            while (graphUpdateQueueRegular.Count > 0)
            {
                ...
                if ((threading & GraphUpdateThreading.UnityInit) != 0)
                {
                    ...
                    if (StartAsyncUpdatesIfQueued()) return false;
                ...
                else
                {
                    ...
                    if ((threading & GraphUpdateThreading.UnityPost) != 0) s.graph.UpdateAreaPost(s.obj);

                    s.obj.internalStage -= 1;
                    Assert.IsTrue(s.obj.internalStage >= 0);
                }
            }
        ...
        /// <summary>
        ///     Signal the graph update thread to start processing graph updates if there are any in the
        ///     <see cref="graphUpdateQueueAsync" /> queue.
...
#if !UNITY_WEBGL
        private readonly AutoResetEvent graphUpdateAsyncEvent = new(false);
        private readonly AutoResetEvent exitAsyncThread = new(false);
#endif
    }
}