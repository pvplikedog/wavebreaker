...
namespace Pathfinding
{
    ...
    ///     Note: Usually you do not want to use this type of link, you want to use NodeLink2 or NodeLink (sorry for the not so
    ///     descriptive names).
    /// </summary>
    ...
    public class NodeLink3 : GraphModifier
    {
        ...
        private static readonly Color GizmosColor = new(206.0f / 255.0f, 136.0f / 255.0f, 48.0f / 255.0f, 0.5f);
        private static readonly Color GizmosColorSelected = new(235.0f / 255.0f, 123.0f / 255.0f, 32.0f / 255.0f, 1.0f);
        ...
        protected override void OnEnable()
        {
            ...
            if (Application.isPlaying && AstarPath.active != null && AstarPath.active.data != null &&
                AstarPath.active.data.pointGraph != null) OnGraphsPostUpdate();
#endif
        }
        ...
        public void OnDrawGizmos(bool selected)
        {
            ...
            if (StartTransform != null) Draw.Gizmos.CircleXZ(StartTransform.position, 0.4f, col);
            if (EndTransform != null) Draw.Gizmos.CircleXZ(EndTransform.position, 0.4f, col);
        ...
        {
#if !ASTAR_NO_POINT_GRAPH
            if (AstarPath.active.data.pointGraph == null) AstarPath.active.data.AddGraph(typeof(PointGraph));
        ...
        public override void OnGraphsPostUpdate()
        {
            if (!AstarPath.active.isScanning)
            {
                if (connectedNode1 != null && connectedNode1.Destroyed) connectedNode1 = null;
                if (connectedNode2 != null && connectedNode2.Destroyed) connectedNode2 = null;
        ...
        public void Apply(bool forceNewCheck)
        {
            ...
            var cost = (uint)Mathf.RoundToInt(((Int3)(StartTransform.position - EndTransform.position)).costMagnitude *
                                              costFactor);
            ...
            for (var a = 0; a < connectedNode1.GetVertexCount(); a++)
            {
                ...
                for (var b = 0; b < connectedNode2.GetVertexCount(); b++)
                {
                    ...
                    if (Int3.Angle(vb2 - vb1, va2 - va1) > 170.0 / 360.0f * Mathf.PI * 2)
                    {
                        ...
                        t2 = Math.Min(t2, VectorMath.ClosestPointOnLineFactor(va1, va2, vb1));
                        t1 = Math.Max(t1, VectorMath.ClosestPointOnLineFactor(va1, va2, vb2));
