...
namespace Pathfinding
{
	...
	public static class GraphUtilities
    {
	    ...
	    ///     Returns: A list of segments. Every 2 elements form a line segment. The first segment is (result[0], result[1]), the
	    ///     second one is (result[2], result[3]) etc.
	    ///     The line segments are oriented so that the navmesh is on the right side of the segments when seen from above.
	    ///     This method works for navmesh, recast, grid graphs and layered grid graphs. For other graph types it will return an
	    ///     empty list.
	    ///     If you need more information about how the contours are connected you can take a look at the other variants of this
	    ///     method.
	    ///     <code>
	    /// // Get the first graph
	    ...
	    ///     [Open online documentation to see images]
	    ///     This image is just used to illustrate the difference between chains and cycles. That it shows a grid graph is not
	    ///     relevant.
	    ///     [Open online documentation to see images]
	    ///     See: <see cref="GetContours(NavGraph)" />
	    /// </summary>
	    /// <param name="navmesh">
	    ///     The navmesh-like object to trace. This can be a recast or navmesh graph or it could be a single
	    ///     tile in one such graph.
	    /// </param>
	    /// <param name="results">
	    ///     Will be called once for each contour with the contour as a parameter as well as a boolean
	    ///     indicating if the contour is a cycle or a chain (see second image).
	    /// </param>
	    public static void GetContours(INavmesh navmesh, Action<List<Int3>, bool> results)
        {
            ...
            {
                var vertices = ListPool<Int3>.Claim();
                for (var i = 0; i < chain.Count; i++) vertices.Add(vertexPositions[chain[i]]);
	    ...
	    /// <param name="callback">
	    ///     The callback will be called once for every contour that is found with the vertices of the
	    ///     contour. The contour always forms a cycle.
	    /// </param>
	    /// <param name="yMergeThreshold">
	    ///     Contours will be simplified if the y coordinates for adjacent vertices differ by no more
	    ///     than this value.
	    /// </param>
	    /// <param name="nodes">
	    ///     Only these nodes will be searched. If this parameter is null then all nodes in the grid graph will
	    ///     be searched.
	    /// </param>
	    public static void GetContours(GridGraph grid, Action<Vector3[]> callback, float yMergeThreshold,
            GridNodeBase[] nodes = null)
        {
            // Set of all allowed nodes or null if all nodes are allowed
            var nodeSet = nodes != null ? new HashSet<GridNodeBase>(nodes) : null;

            // Use all nodes if the nodes parameter is null
            nodes = nodes ?? grid.nodes;
            var neighbourXOffsets = grid.neighbourXOffsets;
            var neighbourZOffsets = grid.neighbourZOffsets;
            ...
            if (nodes != null)
            {
                ...
                for (var i = 0; i < nodes.Length; i++)
                {
                    ...
                    if (startNode != null && startNode.Walkable &&
                        ...
                        for (var startDir = 0; startDir < neighbourIndices.Length; startDir++)
                        {
                            var startState = (startNode.NodeIndex << 4) | startDir;
                                ...
                                !seenStates.Contains(startState))
                            {
                                ...
                                while (true)
                                {
                                    var state = (node.NodeIndex << 4) | dir;
                                    if (state == startState && trace.Count > 0) break;
                                    ...
                                    if (neighbour == null || (nodeSet != null && !nodeSet.Contains(neighbour)))
                                    {
                                        ...
                                        var graphSpacePos = new Vector3(node.XCoordinateInGrid + 0.5f, 0,
                                ...
                                if (trace.Count >= 3)
                                {
                                    ...
                                    if (!(((Mathf.Abs(v1d.x) > 0.01f || Mathf.Abs(v2d.x) > 0.01f) &&
                                           (Mathf.Abs(v1d.z) > 0.01f || Mathf.Abs(v2d.z) > 0.01f)) ||
                                          Mathf.Abs(v1d.y) > yMergeThreshold ||
                                          Mathf.Abs(v2d.y) > yMergeThreshold)) trace.RemoveAt(trace.Count - 1);
                                }
                                ...
                                if (trace.Count >= 3)
                                {
                                    ...
                                    if (!(((Mathf.Abs(v1d.x) > 0.01f || Mathf.Abs(v2d.x) > 0.01f) &&
                                           (Mathf.Abs(v1d.z) > 0.01f || Mathf.Abs(v2d.z) > 0.01f)) ||
                                          Mathf.Abs(v1d.y) > yMergeThreshold ||
                                          Mathf.Abs(v2d.y) > yMergeThreshold)) trace.RemoveAt(0);
                                }
