...
namespace Pathfinding
{
	...
	///     This is for example used when picking the end node of a path. The end node is determined as the closest node to the
	///     end point
	///     that can be reached from the start node.
	///     This data structure's primary purpose is to keep track of which connected component each node is contained in, in
	///     order to make such queries fast.
	...
	///     The connected component is exposed on nodes as the <see cref="Pathfinding.GraphNode.Area" /> property and on this
	///     class using the <see cref="GetArea" /> method.
	...
	///     the connected components were completely recalculated. This can be done relatively efficiently using a flood
	///     filling
	...
	///     Each node in this hierarchical graph represents a larger number of real nodes that are one single connected
	///     component.
	///     Take a look at the image below for an example. In the image each color is a separate hierarchical node, and the
	///     black connections go between the center of each hierarchical node.
	///     [Open online documentation to see images]
	///     With the hierarchical graph, the connected components can be calculated by flood filling the hierarchical graph
	///     instead of the real graph.
	///     Then when we need to know which connected component a node belongs to, we look up the connected component of the
	///     hierarchical node the node belongs to.
	///     The benefit is not immediately obvious. The above is just a bit more complicated way to accomplish the same thing.
	///     However the real benefit comes when updating the graph.
	///     When the graph is updated, all hierarchical nodes which contain any node that was affected by the update is removed
	///     completely and then once all have been removed new hierarchical nodes are recalculated in their place.
	///     Once this is done the connected components of the whole graph can be updated by flood filling only the hierarchical
	///     graph. Since the hierarchical graph is vastly smaller than the real graph, this is significantly faster.
	///     [Open online documentation to see videos]
	///     So finally using all of this, the connected components of the graph can be recalculated very quickly as the graph
	///     is updated.
	///     The effect of this grows larger the larger the graph is, and the smaller the graph update is. Making a small update
	///     to a 1000x1000 grid graph is on the order of 40 times faster with these optimizations.
	///     When scanning a graph or making updates to the whole graph at the same time there is however no speed boost. In
	///     fact due to the extra complexity it is a bit slower, however after profiling the extra time seems to be mostly
	///     insignificant compared to the rest of the cost of scanning the graph.
	///     [Open online documentation to see videos]
	...
	public class HierarchicalGraph
    {
        ...
        private readonly Action<GraphNode> connectionCallback;
        ...
        private readonly Stack<int> freeNodeIndices = new();
        ...
        private readonly Queue<GraphNode> temporaryQueue = new();
        private readonly Stack<int> temporaryStack = new();
        ...
        private void Grow()
        {
            var newChildren = new List<GraphNode>[Math.Max(64, children.Length * 2)];
        ...
        internal void OnCreatedNode(GraphNode node)
        {
            ...
            {
                var newDirty = new GraphNode[Math.Max(node.NodeIndex + 1, dirtyNodes.Length * 2)];
            ...
            if (!node.IsHierarchicalNodeDirty)
            {
                ...
                else
                {
                    ...
                    for (var i = numDirtyNodes - 1; i >= 0; i--)
                        ...
                        {
                            maxIndex = Math.Max(maxIndex, dirtyNodes[i].NodeIndex);
                        }

                    if (numDirtyNodes >= dirtyNodes.Length)
                        throw new Exception("Failed to compactify dirty nodes array. This should not happen. " +
        ...
        private void RemoveHierarchicalNode(int hierarchicalNode, bool removeAdjacentSmallNodes)
        {
            ...
            for (var i = 0; i < nodeChildren.Count; i++) AddDirtyNode(nodeChildren[i]);
        ...
        public void RecalculateIfNecessary()
        {
            if (numDirtyNodes > 0)
            {
                ...
                for (var i = 0; i < numDirtyNodes; i++) dirty[dirtyNodes[i].HierarchicalNodeIndex] = 1;
                ...
                for (var i = 1; i < dirty.Length; i++)
                    if (dirty[i] == 1)
                        RemoveHierarchicalNode(i, true);
                for (var i = 1; i < dirty.Length; i++) dirty[i] = 0;

                for (var i = 0; i < numDirtyNodes; i++) dirtyNodes[i].HierarchicalNodeIndex = 0;
        ...
        /// <summary>
        ///     Flood fills the graph of hierarchical nodes and assigns the same area ID to all hierarchical nodes that are in
        ...
        private void FloodFill()
        {
            ...
            NumConnectedComponents = Math.Max(1, currentArea + 1);
            version++;
        }

        /// <summary>
        ///     Run a BFS out from a start node and assign up to MaxChildrenPerNode nodes to the specified hierarchical node
        ///     which are not already assigned to another hierarchical node
        /// </summary>
        private void FindHierarchicalNodeChildren(int hierarchicalNode, GraphNode startNode)
        {
            ...
            while (que.Count > 0) que.Dequeue().GetConnections(connectionCallback);

            for (var i = 0; i < currentConnections.Count; i++) connections[currentConnections[i]].Add(hierarchicalNode);
