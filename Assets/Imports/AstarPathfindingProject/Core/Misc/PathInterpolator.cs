...
namespace Pathfinding.Util
{
    ...
    public class PathInterpolator
    {
        ...
        public virtual Vector3 position
        {
            get
            {
                var t = currentSegmentLength > 0.0001f
        ...
        {
            if (!valid) throw new Exception("PathInterpolator is not valid");
            buffer.Add(position);
            for (var i = segmentIndex + 1; i < path.Count; i++) buffer.Add(path[i]);
        }
        ...
        public void SetPath(List<Vector3> path)
        {
            ...
            if (path.Count < 2) throw new ArgumentException("Path must have a length of at least 2");
        ...
        {
            if (path == null) return;
            if (index < 0 || index >= path.Count - 1) throw new ArgumentOutOfRangeException("index");
        ...
        public void MoveToLocallyClosestPoint(Vector3 point, bool allowForwards = true, bool allowBackwards = true)
        {
            ...
            while (allowForwards && segmentIndex < path.Count - 2 && (path[segmentIndex + 1] - point).sqrMagnitude <=
                   (path[segmentIndex] - point).sqrMagnitude) NextSegment();

            while (allowBackwards && segmentIndex > 0 && (path[segmentIndex - 1] - point).sqrMagnitude <=
                   (path[segmentIndex] - point).sqrMagnitude) PrevSegment();
        ...
        public void MoveToCircleIntersection2D(Vector3 circleCenter3D, float radius, IMovementPlane transform)
        {
            ...
            while (segmentIndex < path.Count - 2 &&
                   (transform.ToPlane(path[segmentIndex + 1]) - circleCenter).sqrMagnitude <= radius * radius)
                NextSegment();

            // Calculate the intersection with the circle. This involves some math.
            var factor = VectorMath.LineCircleIntersectionFactor(circleCenter, transform.ToPlane(path[segmentIndex]),
                transform.ToPlane(path[segmentIndex + 1]), radius);
