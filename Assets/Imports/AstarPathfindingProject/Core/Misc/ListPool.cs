	...
	public static class ListPool<T>
    {
        ...
        private static int FindCandidate(List<List<T>> pool, int capacity)
        {
            ...
            for (var i = 0; i < pool.Count && i < MaxCapacitySearchLength; i++)
            {
                ...
                if ((list == null || candidate.Capacity > list.Capacity) && candidate.Capacity < capacity * 16)
                {
                    ...
                    if (list.Capacity >= capacity) return listIndex;
                }
            }
        ...
        public static List<T> Claim(int capacity)
        {
            ...
            lock (pool)
            {
                ...
                if (listIndex == -1) return new List<T>(capacity);
            ...
            lock (pool)
            {
                ...
                if (!inPool.Add(list))
                    throw new InvalidOperationException(
                        "You are trying to pool a list twice. Please make sure that you only pool it once.");
#endif
                if (list.Capacity > LargeThreshold)
                {
                    ...
                    if (largePool.Count > MaxLargePoolSize) largePool.RemoveAt(0);
                }
                else
                {
                    pool.Add(list);
...
#if !ASTAR_NO_POOLING
        private static readonly List<List<T>> largePool = new();
        private static readonly HashSet<List<T>> inPool = new();
#endif
    }
}