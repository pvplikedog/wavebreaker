...
namespace Pathfinding
{
    ...
    {
        private static readonly Dictionary<Type, Stack<Path>> pool = new();
        private static readonly Dictionary<Type, int> totalCreated = new();
        ...
        public static void Pool(Path path)
        {
            ...
            lock (pool)
            {
                if (((IPathInternals)path).Pooled) throw new ArgumentException("The path is already pooled.");
        ...
        public static int GetTotalCreated(Type type)
        {
            ...
            if (totalCreated.TryGetValue(type, out created)) return created;
        ...
        public static int GetSize(Type type)
        {
            ...
            if (pool.TryGetValue(type, out poolStack)) return poolStack.Count;
        ...
        public static T GetPath<T>() where T : Path, new()
        {
            ...
            lock (pool)
            {
                ...
                else
                {
                    ...
                    if (!totalCreated.ContainsKey(typeof(T))) totalCreated[typeof(T)] = 0;
