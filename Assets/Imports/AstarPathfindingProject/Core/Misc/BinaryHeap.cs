...
namespace Pathfinding
{
	...
	public class BinaryHeap
    {
        ...
        private void Expand()
        {
            ...
            var newSize = Math.Max(heap.Length + 4, Math.Min(65533, (int)Math.Round(heap.Length * growthFactor)));
        ...
        private void Validate()
        {
            for (var i = 1; i < numberOfItems; i++)
            {
                ...
                if (heap[parentIndex].F > heap[i].F)
                    throw new Exception("Invalid state at " + i + ":" + parentIndex + " ( " + heap[parentIndex].F +
                                        " > " + heap[i].F + " ) ");
#if DECREASE_KEY
                if (heap[i].node.heapIndex != i) throw new Exception("Invalid heap index");
#endif
            }
        }
