...
namespace Pathfinding
{
    ...
    public class StartEndModifier : PathModifier
    {
	    ...
	    ///     The images above show a path coming in from the top left corner and ending at a node next to an obstacle as well as
	    ///     2 different possible end points of the path and how they would be modified.
	    ///     [Open online documentation to see images]
	    /// </summary>
	    public enum Exactness
        {
	        ...
	        ///     If you recalculate the path while the agent is moving you may want the start point snapping to be ClosestOnNode and
	        ///     the end point snapping to be SnapToNode however
	        ...
	        ///     Note that if a path was for example requested to a point inside an obstacle, then the last point of the path will
	        ///     be inside that obstacle, which is usually not what you want.
	        ...
	        ///     Usually you will want to use the NodeConnection mode instead since that is usually the behaviour that you really
	        ///     want.
	        ...
	        /// <summary>
	        ///     The point is set to the closest point on the surface of the node. Note that some node types (point nodes) do not
	        ///     have a surface, so the "closest point" is simply the node's position which makes this identical to
	        ///     <see cref="Exactness.SnapToNode" />.
	        ...
	        ///     Note: If you are using this mode with a <see cref="Pathfinding.PointGraph" /> you probably also want to use the
	        ///     <see cref="Pathfinding.PointGraph.NodeDistanceMode Connection" /> for
	        ///     <see cref="Pathfinding.PointGraph.nearestNodeDistanceMode" />.
	        ///     [Open online documentation to see images]
	        /// </summary>
	        NodeConnection
        }
	    ...
	    ///     Version: Since 4.1 this field only has an effect for the <see cref="Exactness" /> mode Original because that's the
	    ///     only one where it makes sense.
	    /// </summary>
        ...
        private Vector3 Snap(ABPath path, Exactness mode, bool start, out bool forceAddPoint,
            out int closestConnectionIndex)
        {
                ...
                case Exactness.NodeConnection:
                    ...
                    switch (mode)
                    {
                        ...
                        case Exactness.NodeConnection:
                            ...
                            connectionBufferAddDelegate = connectionBufferAddDelegate ?? connectionBuffer.Add;
        ...
        protected Vector3 GetClampedPoint(Vector3 from, Vector3 to, GraphNode hint)
        {
            ...
            if (useGraphRaycasting && hint != null)
            {
                ...
                if (rayGraph != null)
                {
                    ...
                    if (rayGraph.Linecast(from, point, hint, out graphHit)) point = graphHit.point;
                }
            }
