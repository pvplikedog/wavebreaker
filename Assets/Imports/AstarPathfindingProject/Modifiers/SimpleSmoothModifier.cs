...
namespace Pathfinding
{
    ...
    public class SimpleSmoothModifier : MonoModifier
    {
        ...
        /// <summary>
        ///     Determines how much smoothing to apply in each smooth iteration. 0.5 usually produces the nicest looking
        ///     curves.
        /// </summary>
        [Tooltip(
        ...
        public List<Vector3> CurvedNonuniform(List<Vector3> path)
        {
            ...
            for (var i = 0; i < path.Count - 1; i++)
            {
                ...
                for (float t = 0; t <= dist; t += maxSegmentLength) pointCounter++;
            }
        ...
        {
            if (path.Count <= 2 || iterations <= 0) return path;
            ...
            for (var i = 0; i < path.Count; i++) subdivided[i] = path[i];
        ...
        public List<Vector3> SmoothSimple(List<Vector3> path)
        {
            ...
            if (uniformLength)
            {
                ...
                float pathLength = 0;
                for (var i = 0; i < path.Count - 1; i++) pathLength += Vector3.Distance(path[i], path[i + 1]);
