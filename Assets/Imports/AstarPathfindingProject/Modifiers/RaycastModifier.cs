...
namespace Pathfinding
{
	...
	///     \ingroup modifiers
	///     This modifier will try to remove as many nodes as possible from the path using raycasting (linecasting) to validate
	///     the node removal.
	///     You can use either graph raycasting or Physics.Raycast.
	///     When using graph raycasting, the graph will be traversed and checked for obstacles. When physics raycasting is
	///     used, the Unity physics system
	...
	///     This modifier is primarily intended for grid graphs and layered grid graphs. Though depending on your game it may
	///     also be
	///     useful for point graphs. However note that point graphs do not have any built-in raycasting so you need to use
	///     physics raycasting for that graph.
	///     For navmesh/recast graphs the <see cref="Pathfinding.FunnelModifier" /> is a much better and faster alternative.
	///     On grid graphs you can combine the FunnelModifier with this modifier by simply attaching both of them to a
	///     GameObject with a Seeker.
	///     This may or may not give you better results. It will usually follow the border of the graph more closely when they
	///     are both used
	///     however it more often misses some simplification opportunities.
	///     When both modifiers are used then the funnel modifier will run first and simplify the path, and then this modifier
	///     will take
	...
	///     lowest quality level (10 times slower is not unusual). So make sure you pick the lowest quality that your game can
	///     get away with.
	///     You can use the Unity profiler to see if it takes up any significant amount of time. It will show up under the
	///     heading "Running Path Modifiers".
    ...
    public class RaycastModifier : MonoModifier
    {
        ...
        private static readonly int[] iterationsByQuality = { 1, 2, 1, 3 };
        private static List<Vector3> buffer = new();
        ...
        ///     If you are using a grid graph you usually want this to be the same as the mask in the grid graph's 'Collision
        ///     Testing' settings.
        /// </summary>
        ...
        ///     Make sure the ground is either too far below or is not inside the mask since otherwise the raycast might always hit
        ///     the ground.
        ...
        private Filter cachedFilter = new();
        ...
        private List<Vector3> ApplyGreedy(Path p, List<Vector3> points, Func<GraphNode, bool> filter,
        ...
        private List<Vector3> ApplyDP(Path p, List<Vector3> points, Func<GraphNode, bool> filter,
            NNConstraint nnConstraint)
        {
        ...
        ///     If both n1 and n2 are supplied it is assumed that the line goes from the center of n1 to the center of n2 and a
        ///     more optimized graph linecast may be done.
        /// </summary>
        protected bool ValidateLine(GraphNode n1, GraphNode n2, Vector3 v1, Vector3 v2, Func<GraphNode, bool> filter,
            NNConstraint nnConstraint)
        {
            if (useRaycasting)
            {
                ...
                if (use2DPhysics)
                {
                    if (thickRaycast && thickRaycastRadius > 0 && Physics2D.CircleCast(v1 + raycastOffset,
                            thickRaycastRadius, v2 - v1, (v2 - v1).magnitude, mask)) return false;

                    if (Physics2D.Linecast(v1 + raycastOffset, v2 + raycastOffset, mask)) return false;
                }
                else
                {
                    ...
                    if (Physics.Linecast(v1 + raycastOffset, v2 + raycastOffset, mask)) return false;
            ...
            if (useGraphRaycasting)
            {
                ...
                if (n1 != null && n2 != null)
                {
                    ...
                    if (graph != graph2) return false;

                    var rayGraph = graph as IRaycastableGraph;
                    if (rayGraph != null) return !rayGraph.Linecast(v1, v2, out var _, null, filter);
                }
            }
