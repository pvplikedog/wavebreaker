...
namespace Pathfinding
{
	...
	public class Funnel
    {
        public static List<PathPart> SplitIntoParts(Path path)
        {
            ...
            for (var i = 0; i < nodes.Count; i++)
                ...
                else if (NodeLink2.GetNodeLink(nodes[i]) != null)
                {
                    ...
                    if (i - part.startIndex != 1)
                        throw new Exception("NodeLink2 link length greater than two (2) nodes. " +
                                            (i - part.startIndex + 1));
        ...
        private static bool UnwrapHelper(Vector3 portalStart, Vector3 portalEnd, Vector3 prevPoint, Vector3 nextPoint,
            ref Quaternion mRot, ref Vector3 mOffset)
        {
            // Skip the point if it was on the rotation axis
            if (VectorMath.IsColinear(portalStart, portalEnd, nextPoint)) return false;
        ...
        ///     The result is stored in the left and right arrays which must be at least as large as the funnel.left and
        ///     funnel.right lists.
        ///     The input is a funnel like in the image below. It may be rotated and twisted.
        ///     [Open online documentation to see images]
        ///     The output will be a funnel in 2D space like in the image below. All twists and bends will have been straightened
        ///     out.
        ///     [Open online documentation to see images]
        ...
        private static int FixFunnel(Vector2[] left, Vector2[] right, int numPortals)
        {
            ...
            while (left[startIndex + 1] == left[startIndex + 2] && right[startIndex + 1] == right[startIndex + 2])
            {
                ...
                if (numPortals - startIndex < 3) return -1;
            }
        ...
        /// </summary>
        /// <param name="funnel">
        ///     The portals of the funnel. The first and last vertices portals must be single points (so for
        ...
        /// <param name="splitAtEveryPortal">
        ///     If true, then a vertex will be inserted every time the path crosses a portal
        ...
        public static List<Vector3> Calculate(FunnelPortals funnel, bool unwrap, bool splitAtEveryPortal)
        {
            ...
            for (var i = 0; i < intermediateResult.Count; i++)
            {
                ...
                if (splitAtEveryPortal)
                {
                    ...
                    for (var j = prevIdx + 1; j < Math.Abs(idx); j++)
                    {
        ...
        ///     The result is the indices of the vertices that were picked, a non-negative value refers to the corresponding index
        ///     in the
        ...
        ///     See: http://digestingduck.blogspot.se/2010/03/simple-stupid-funnel-algorithm.html
        /// </summary>
        private static void Calculate(Vector2[] left, Vector2[] right, int numPortals, int startIndex,
            List<int> funnelPath, int maxCorners, out bool lastCorner)
        {
            ...
            {
                if (funnelPath.Count >= maxCorners) return;
