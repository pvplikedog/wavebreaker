...
namespace Pathfinding
{
	...
	///     An update will be triggered whenever the bounding box of the attached collider has changed (moved/expanded/etc.) by
	///     at least <see cref="updateError" /> world units or if
	///     the GameObject has rotated enough so that the outmost point of the object has moved at least
	///     <see cref="updateError" /> world units.
	...
	///     You can use this with recast graphs as well. However since recast graph updates are much slower it is recommended
	///     to use the <see cref="Pathfinding.NavmeshCut" /> component if at all possible.
    ...
    public class DynamicGridObstacle : GraphModifier
    {
        ...
        private Bounds bounds
        {
            get
            {
                if (coll != null) return coll.bounds;
        ...
        private bool colliderEnabled => coll != null ? coll.enabled : coll2D.enabled;
        ...
        protected override void Awake()
        {
            ...
            if (coll == null && coll2D == null && Application.isPlaying)
                throw new Exception("A collider or 2D collider must be attached to the GameObject(" + gameObject.name +
        ...
        private void Update()
        {
            ...
            if (AstarPath.active == null || AstarPath.active.isScanning ||
                Time.realtimeSinceStartup - lastCheckTime < checkTime || !Application.isPlaying ||
                pendingGraphUpdates.Count > 0) return;
            ...
            if (colliderEnabled)
            {
                ...
                if (minDiff.sqrMagnitude > updateError * updateError ||
            ...
            else
            {
                ...
                if (prevEnabled) DoUpdateGraphs();
            }
        }

        /// <summary>
        ///     Revert graphs when disabled.
        ///     When the DynamicObstacle is disabled or destroyed, a last graph update should be done to revert nodes to their
        ///     original state
        /// </summary>
