...
namespace Pathfinding
{
	...
	///     This class works a lot with the <see cref="Pathfinding.GraphNode" /> class, a useful function to get nodes is
	...
	public static class PathUtilities
    {
        ...
        ///     This function performs a DFS (depth-first-search) or flood fill of the graph and returns all nodes which can be
        ///     reached from
        ///     the seed node. In almost all cases this will be identical to returning all nodes which have the same area as the
        ///     seed node.
        ...
        ///     Returns: A List
        ///     <Node>
        ///         containing all nodes reachable from the seed node.
        ...
        /// <param name="filter">
        ///     Optional filter for which nodes to search. You can combine this with tagMask = -1 to make the filter determine
        ///     everything.
        ///     Only walkable nodes are searched regardless of the filter. If the filter function returns false the node will be
        ///     treated as unwalkable.
        /// </param>
        public static List<GraphNode> GetReachableNodes(GraphNode seed, int tagMask = -1,
            Func<GraphNode, bool> filter = null)
        {
            ...
            while (dfsStack.Count > 0) dfsStack.Pop().GetConnections(callback);
        ...
        ///     This function performs a BFS (<a href="https://en.wikipedia.org/wiki/Breadth-first_search">breadth-first search</a>
        ///     ) or flood fill of the graph and returns all nodes within a specified node distance which can be reached from
        ///     the seed node. In almost all cases when depth is large enough this will be identical to returning all nodes which
        ///     have the same area as the seed node.
        ///     In the editor areas are displayed as different colors of the nodes.
        ///     The only case where it will not be so is when there is a one way path from some part of the area to the seed node
        ///     but no path from the seed node to that part of the graph.
        ...
        ///     Returns: A List
        ///     <GraphNode>
        ///         containing all nodes reachable up to a specified node distance from the seed node.
        ///         For better memory management the returned list should be pooled, see Pathfinding.Util.ListPool
        ///         Warning: This method is not thread safe. Only use it from the Unity thread (i.e normal game code).
        ///         The video below shows the BFS result with varying values of depth. Points are sampled on the nodes using
        ///         <see cref="GetPointsOnNodes" />.
        ...
        /// <param name="tagMask">Optional mask for tags. This is a bitmask.</param>
        /// <param name="filter">
        ///     Optional filter for which nodes to search. You can combine this with depth = int.MaxValue and tagMask = -1 to make
        ///     the filter determine everything.
        ///     Only walkable nodes are searched regardless of the filter. If the filter function returns false the node will be
        ///     treated as unwalkable.
        /// </param>
        ...
        /// <param name="g">
        ///     The graph to use for linecasting. If you are only using one graph, you can get this by AstarPath.active.graphs[0]
        ///     as IRaycastableGraph.
        ///     Note that not all graphs are raycastable, recast, navmesh and grid graphs are raycastable. On recast and navmesh it
        ///     works the best.
        /// </param>
        /// <param name="previousPoints">
        ///     The points to use for reference. Note that these are in world space.
        ...
        /// <summary>
        ///     Will calculate a number of points around center which are on the graph and are separated by clearance from each
        ///     other.
        ...
        ///     This is great if you want to pick a number of target points for group movement. If you pass all current agent
        ///     points from e.g the group's average position
        ///     this method will return target points so that the units move very little within the group, this is often
        ///     aesthetically pleasing and reduces jitter if using
        ...
        /// </param>
        /// <param name="previousPoints">
        ///     The points to use for reference. Note that these should not be in world space. They are treated as relative to
        ///     center.
        ///     The new points will overwrite the existing points in the list. The result will be in world space, not relative to
        ///     center.
        /// </param>
        ...
        /// <summary>
        ///     Returns randomly selected points on the specified nodes with each point being separated by clearanceRadius from
        ///     each other.
        ///     Selecting points ON the nodes only works for TriangleMeshNode (used by Recast Graph and Navmesh Graph) and GridNode
        ///     (used by GridGraph).
        ...
        ///     which assumes that all nodes in the list have the same surface area (which usually is a surface area of zero and
        ///     the
        ...
        public static List<Vector3> GetPointsOnNodes(List<GraphNode> nodes, int count, float clearanceRadius = 0)
        {
            ...
            else
            {
                ...
                for (var i = 0; i < count; i++) pts.Add(nodes[Random.Range(0, nodes.Count)].RandomPointOnSurface());
            }
