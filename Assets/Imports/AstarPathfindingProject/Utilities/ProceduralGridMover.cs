...
namespace Pathfinding
{
	...
	///     It makes it possible to have vast open worlds (maybe procedurally generated) and still be able to use pathfinding
	///     on them.
    ...
    public class ProceduralGridMover : VersionedMonoBehaviour
    {
        ...
        /// </summary>
        [HideInInspector] public int graphIndex;
        ...
        private void Start()
        {
            ...
            if (graph == null)
            {
                if (graphIndex < 0) throw new Exception("Graph index should not be negative");
                if (graphIndex >= AstarPath.active.data.graphs.Length)
                    throw new Exception("The ProceduralGridMover was configured to use graph index " + graphIndex +
                                        ", but only " + AstarPath.active.data.graphs.Length + " graphs exist");

                graph = AstarPath.active.data.graphs[graphIndex] as GridGraph;
                if (graph == null)
                    throw new Exception("The ProceduralGridMover was configured to use graph index " + graphIndex +
                                        " but that graph either does not exist or is not a GridGraph or LayerGridGraph");
            }
        ...
        private void Update()
        {
            ...
            if (VectorMath.SqrDistanceXZ(graphCenterInGraphSpace, targetPositionInGraphSpace) >
                updateDistance * updateDistance) UpdateGraph();
        }
        ...
        private IEnumerator UpdateGraphCoroutine()
        {
            ...
            if (buffer == null || buffer.Length != width * depth) buffer = new GridNodeBase[width * depth];
            ...
            if (Mathf.Abs(offset.x) <= width && Mathf.Abs(offset.y) <= depth)
            {
                ...
                var approxNumNodesToUpdate =
                    Mathf.Max(Mathf.Abs(offset.x), Mathf.Abs(offset.y)) * Mathf.Max(width, depth);
                ...
                for (var z = 0; z < depth; z++)
                {
                    ...
                    for (var x = xmin; x < xmax; x++) graph.RecalculateCell(x, z, false, false);
                ...
                for (var z = 0; z < depth; z++)
                {
                    ...
                    for (var x = xmin; x < xmax; x++) graph.CalculateConnections(x, z);
            ...
            else
            {
                ...
                for (var z = 0; z < depth; z++)
                {
                    for (var x = 0; x < width; x++) graph.RecalculateCell(x, z);
                    counter += width;
                    if (counter > yieldEvery)
                    {
                        counter = 0;
                ...
                for (var z = 0; z < depth; z++)
                {
                    for (var x = 0; x < width; x++) graph.CalculateConnections(x, z);
                    counter += width;
                    if (counter > yieldEvery)
                    {
                        counter = 0;
