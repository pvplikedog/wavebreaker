...
namespace Pathfinding.Legacy
{
	...
	///     make it easier
	///     to set up movement for the characters in your game. This script is not written for high performance, so I do not
	///     recommend using it for large groups of units.
	///     \n
	///     \n
	///     This script will try to follow a target transform, in regular intervals, the path to that target will be
	///     recalculated.
	...
	///     The <see cref="target" /> variable is where the AI will try to move, it can be a point on the ground where the
	///     player has clicked in an RTS for example.
	///     Or it can be the player object in a zombie game.\n
	///     The speed is self-explanatory, so is turningSpeed, however <see cref="slowdownDistance" /> might require some
	///     explanation.
	///     It is the approximate distance from the target where the AI will start to slow down. Note that this doesn't only
	///     affect the end point of the path
	///     but also any intermediate points, so be sure to set <see cref="forwardLook" /> and
	///     <see cref="pickNextWaypointDist" /> to a higher value than this.\n
	///     <see cref="pickNextWaypointDist" /> is simply determines within what range it will switch to target the next
	///     waypoint in the path.\n
	///     <see cref="forwardLook" /> will try to calculate an interpolated target point on the current segment in the path so
	///     that it has a distance of <see cref="forwardLook" /> from the AI\n
	///     Below is an image illustrating several variables as well as some internal ones, but which are relevant for
	///     understanding how it works.
	///     Note that the <see cref="forwardLook" /> range will not match up exactly with the target point practically, even
	///     though that's the goal.
	///     [Open online documentation to see images]
	///     This script has many movement fallbacks.
	///     If it finds a NavmeshController, it will use that, otherwise it will look for a character controller, then for a
	///     rigidbody and if it hasn't been able to find any
    ...
    {
	    /// <summary>
	    ///     Target point is Interpolated on the current segment in the path so that it has a distance of
	    ///     <see cref="forwardLook" /> from the AI.
	    ...
	    ///     Usually the AI has moved a bit between requesting the path, and getting it back, and there is usually a small gap
	    ///     between the AI
	    ///     and the closest node.
	    ///     If this option is enabled, it will simulate, when the path callback is received, movement between the closest node
	    ///     and the current
	    ///     AI position. This helps to reduce the moments when the AI just get a new path back, and thinks it ought to move
	    ///     backwards to the start of the new path
        ...
        ///     A path is first requested by <see cref="SearchPath" />, it is then calculated, probably in the same or the next
        ///     frame.
        ...
        ///     A whole bunch of restrictions on the velocity is applied to make sure it doesn't overshoot, does not look too far
        ///     ahead,
        ...
        protected new Vector3 CalculateVelocity(Vector3 currentPosition)
        {
            ...
            if (vPath.Count == 1) vPath.Insert(0, currentPosition);
            ...
            var targetPosition = CalculateTargetPoint(currentPosition, vPath[currentWaypointIndex - 1],
                vPath[currentWaypointIndex]);
            ...
            if (Time.deltaTime > 0) sp = Mathf.Clamp(sp, 0, targetDist / (Time.deltaTime * 2));
