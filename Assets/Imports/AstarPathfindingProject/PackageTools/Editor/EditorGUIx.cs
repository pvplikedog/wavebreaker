...
namespace Pathfinding
{
    ...
    public class FadeArea
    {
        ...
        private readonly GUIStyle areaStyle;
        ...
        private void Tick()
        {
            if (Event.current.type == EventType.Repaint)
            {
                ...
                if (!Mathf.Approximately(targetValue, value))
                {
                    ...
                    if (!fancyEffects) value = targetValue;
                }
                else
                {
                    value = targetValue;
    ...
    {
        private static readonly Dictionary<int, string[]> layerNames = new();
        ...
        public static int TagField(string label, int value, Action editCallback)
        {
            ...
            var newValue = EditorGUILayout.IntPopup(label, value, tagNamesAndEditTagsButton,
                new[]
                {
        ...
        {
            if (tagMask == null) throw new ArgumentNullException("tagMask");
            if (EditorGUILayout.Foldout(foldout, label))
            {
                EditorGUI.indentLevel++;
                GUILayout.BeginVertical();
                for (var i = 0; i < tagMask.Count; i++) tagMask[i] = EditorGUILayout.TagField(tagMask[i]);
        ...
        {
            if (Event.current.type == EventType.Layout && DateTime.UtcNow.Ticks - lastUpdateTick > 10000000L)
            {
                layerNames.Clear();
