...
namespace Pathfinding.Examples
{
	...
	///     Deprecated: This script has been replaced by Pathfinding.Examples.MineBotAnimation. Any uses of this script in the
	///     Unity editor will be automatically replaced by one AIPath component and one MineBotAnimation component.
	/// </summary>
    ...
    public class MineBotAI : AIPath
    {
        ...
        protected override int OnUpgradeSerializedData(int version, bool unityThread)
        {
            if (unityThread)
            {
                ...
                var index = Array.IndexOf(components, this);
                foreach (var newType in new[] { typeof(AIPath), typeof(MineBotAnimation) })
                {
                    var newComp = gameObject.AddComponent(newType);
                    foreach (var field in newComp.GetType().GetFields(BindingFlags.Instance | BindingFlags.Public))
                    {
                        var oldField = GetType().GetField(field.Name);
                        try
                        {
                    ...
                    for (var i = components.Length - 1; i > index; i--) ComponentUtility.MoveComponentUp(newComp);
                }
