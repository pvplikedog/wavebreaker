...
namespace Pathfinding
{
    ...
    public class BaseAIEditor : EditorBase
    {
        ...
        protected override void Inspector()
        {
            ...
            mono.TryGetComponent(out Rigidbody rigid);
            mono.TryGetComponent(out Rigidbody2D rigid2D);
            ...
            {
                EditorGUI.BeginChangeCheck();
                var grav = gravity.hasMultipleDifferentValues ? -1 :
                    gravity.vector3Value == Vector3.zero ? 0 :
                    float.IsNaN(gravity.vector3Value.x) ? 1 : 2;
                ...
                if (!gravity.hasMultipleDifferentValues)
                {
                    ...
                    {
                        EditorGUI.indentLevel++;
                        if (!float.IsNaN(gravity.vector3Value.x)) PropertyField(gravity.propertyPath);
