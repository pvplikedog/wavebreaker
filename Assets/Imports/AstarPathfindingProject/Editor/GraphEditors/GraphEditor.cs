...
namespace Pathfinding
{
    public class GraphEditor : GraphEditorBase
    {
        ...
        ///     Will only scan graphs if not playing and time to scan last graph was less than some constant (to avoid lag with
        ///     large graphs)
        /// </summary>
                    ...
                    if (assetsMustBeInResourcesFolder)
                    {
                        ...
                        if (!AssetDatabase.IsMainAsset(obj) && obj.name != AssetDatabase.LoadMainAssetAtPath(path).name)
                                    ...
                                    "Due to technical reasons, the main asset must\nhave the same name as the referenced asset"))
                            {
                                ...
                                if (error != "") Debug.LogError("Couldn't rename asset - " + error);
                            }
                    }
                }
            }
        ...
        {
            var penalty = EditorGUILayout.IntField(
        ...
        public static void Separator()
        {
            ...
            if (Event.current.type == EventType.Repaint) separator.Draw(r, false, false, false, false);
        }

        /// <summary>
        ///     Draws a small help box with a 'Fix' button to the right. Returns: Boolean - Returns true if the button was
        ///     clicked
        /// </summary>
        ...
        public static bool ToggleGroup(GUIContent label, bool value)
        {
            ...
            if (GUILayout.Button(label, boxHeader, GUILayout.Width(100))) value = !value;
