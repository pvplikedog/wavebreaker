...
namespace Pathfinding
{
    ...
    public class GridGraphEditor : GraphEditor
    {
        ...
        private static readonly Vector3[] handlePoints =
            { new(0.0f, 0, 0.5f), new(1.0f, 0, 0.5f), new(0.5f, 0, 0.0f), new(0.5f, 0, 1.0f) };

        private readonly Vector3[] arcBuffer = new Vector3[21];

        [JsonMember] public bool collisionPreviewOpen = true;

        private readonly GUIContent[] hexagonSizeContents =
        {
        ...
        private void DrawInspectorMode(GridGraph graph)
        {
            ...
            var newMode = (InspectorGridMode)EditorGUILayout.EnumPopup("Shape", graph.inspectorGridMode);
        ...
        private void DrawFirstSection(GridGraph graph)
        {
            ...
            if (graph.inspectorGridMode == InspectorGridMode.Hexagonal)
            {
                ...
                hexagonSize = (float)Math.Round(hexagonSize, 5);
        ...
        private void DrawRotationField(GridGraph graph)
        {
            if (graph.is2D)
            {
                ...
                if (EditorGUI.EndChangeCheck()) graph.rotation = RoundVector3(new Vector3(-90 + angle, 270, 90));
            }
            else
            {
        ...
        private void DrawPositionField(GridGraph graph)
        {
            ...
            if (delta.magnitude > 0.001f) graph.center += delta;
        ...
        protected virtual void DrawNeighbours(GridGraph graph)
        {
            ...
            if (graph.neighbours == NumNeighbours.Eight) DrawCutCorners(graph);
        ...
        private void DrawCollisionPreview(GraphCollision collision)
        {
            ...
            var gridWidthInNodes = collision.type == ColliderType.Ray
                ? 3
            ...
            var nodeSize = (target as GridGraph).nodeSize;
        ...
        public static GridPivot PivotPointSelector(GridPivot pivot)
        {
            ...
            if (gridPivotSelectBackground == null) return pivot;

            if (Event.current.type == EventType.Repaint) gridPivotSelectBackground.Draw(r, false, false, false, false);
        ...
        public override void OnSceneGUI(NavGraph target)
        {
            ...
            if (e.type == EventType.MouseDown)
                isMouseDown = true;
            else if (e.type == EventType.MouseUp) isMouseDown = false;
            ...
            if (Tools.current == Tool.Scale)
            {
                ...
                for (var i = 0; i < handlePoints.Length; i++)
                {
                    ...
                    var p = savedTransform.InverseTransform(Handles.Slider(ps, ps - center,
            ...
            else if (Tools.current == Tool.Move)
            {
                ...
                if (EditorGUI.EndChangeCheck() && Tools.viewTool != ViewTool.Orbit) graph.center = center;
            }
            else if (Tools.current == Tool.Rotate)
            {
                ...
                if (EditorGUI.EndChangeCheck() && Tools.viewTool != ViewTool.Orbit) graph.rotation = rot.eulerAngles;
            }
