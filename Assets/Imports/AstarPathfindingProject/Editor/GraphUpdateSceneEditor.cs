...
namespace Pathfinding
{
    ...
    public class GraphUpdateSceneEditor : EditorBase
    {
        ...
        private static readonly Color PointColor = new(1, 0.36F, 0, 0.6F);
        private static readonly Color PointSelectedColor = new(1, 0.24F, 0, 1.0F);
        ...
        public void OnSceneGUI()
        {
            ...
            var points = ListPool<Vector3>.Claim();
        ...
        protected override void Inspector()
        {
            ...
            for (var i = 0; i < scripts.Length; i++) scripts[i].points = scripts[i].points ?? new Vector3[0];
            ...
            if (points.hasMultipleDifferentValues || points.arraySize > 0) FloatField("minBoundsHeight", min: 0.1f);
            ...
            {
                for (var i = 0; i < scripts.Length; i++) EditorUtility.SetDirty(scripts[i]);
        ...
        private void DrawPointsField()
        {
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                for (var i = 0; i < scripts.Length; i++) scripts[i].RecalcConvex();
                HandleUtility.Repaint();
            }
        }
        ...
        private void DrawTagField()
        {
            if (PropertyField("modifyTag"))
            {
                ...
                if (EditorGUI.EndChangeCheck()) tagValue.intValue = newTag;

                if (GUILayout.Button(
                        "Tags can be used to restrict which units can walk on what ground. Click here for more info",
                        "HelpBox")) Application.OpenURL(AstarUpdateChecker.GetURL("tags"));
                EditorGUI.indentLevel--;
            }
        }
