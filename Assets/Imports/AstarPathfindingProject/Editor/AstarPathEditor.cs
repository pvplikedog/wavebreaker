...
namespace Pathfinding
{
    ...
    public class AstarPathEditor : Editor
    {
        ...
        private static Dictionary<string, CustomGraphEditorAttribute> graphEditorTypes = new();
        ...
        private Type[] graphTypes => script.data.graphTypes;
        ...
        public void OnEnable()
        {
            ...
            if ((!Application.isPlaying &&
                ...
                script.data.graphs == null) LoadGraphs();
        ...
        public void OnSceneGUI()
        {
            ...
            for (var i = 0; i < script.graphs.Length; i++)
            {
                ...
                if (graph != null) graphEditors[i].OnSceneGUI(graph);
            }
        ...
        /// <summary>
        ///     Checks if JS support is enabled. This is done by checking if the directory
        ///     'Assets/AstarPathfindingEditor/Editor' exists
        /// </summary>
        private static bool IsJsEnabled()
        {
            return Directory.Exists(Application.dataPath + "/AstarPathfindingEditor/Editor");
        }
        ...
        private static void EnableJs()
        {
            ...
            if (!Directory.Exists(projectPath + scriptsFolder))
            {
                var error =
            ...
            if (!Directory.Exists(Application.dataPath + "/AstarPathfindingEditor"))
            {
                Directory.CreateDirectory(Application.dataPath + "/AstarPathfindingEditor");
        ...
        {
            if (Directory.Exists(Application.dataPath + "/Plugins/AstarPathfindingProject"))
            {
                ...
                if (error != "")
                    ...
                    try
                    {
                        Directory.Delete(Application.dataPath + "/Plugins");
                    }
                    catch (Exception)
                    {
                    }
            ...
            if (Directory.Exists(Application.dataPath + "/AstarPathfindingEditor/Editor"))
            {
                ...
                if (error != "")
                    ...
                    try
                    {
                        Directory.Delete(Application.dataPath + "/AstarPathfindingEditor");
                    }
                    catch (Exception)
                    {
                    }
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (GUILayout.Button(new GUIContent("Scan",
                    "Recalculate all graphs. Shortcut cmd+alt+s ( ctrl+alt+s on windows )"))) MenuScan();
        ...
        private static bool LoadStyles()
        {
            ...
            if (!EditorResourceHelper.LocateEditorAssets()) return false;
            ...
            if (level0AreaStyle == null) return false;
        ...
        private void DrawOptimizationSettings()
        {
            ...
            if (optimizationSettingsArea.BeginFade())
            {
                ...
                if (GUILayout.Button(
                        ...
                        helpBox)) Application.OpenURL(AstarUpdateChecker.GetURL("astarpro"));
                GUIUtilityx.PopTint();
            }
        ...
        {
            return new Version(Mathf.Max(v.Major, 0), Mathf.Max(v.Minor, 0), Mathf.Max(v.Build, 0),
        ...
        private void DrawAboutArea()
        {
            ...
            if (FullyDefinedVersion(newVersion) > FullyDefinedVersion(AstarPath.Version))
            {
                ...
                if (GUILayout.Button((beta ? "Beta" : "New") + " Version Available! " + newVersion, thinHelpBox,
                        GUILayout.Height(15))) Application.OpenURL(AstarUpdateChecker.GetURL("download"));
                GUIUtilityx.PopTint();
                GUILayout.Space(20);
            }
#endif
        ...
        private void DrawGraphHeader(GraphEditor graphEditor)
        {
            ...
            else
            {
                ...
                if (GUILayout.Button(graph.name, level1LabelStyle))
                {
                    ...
                    if (!graph.open) graph.infoScreenOpen = false;
                    RepaintSceneView();
                }
            }
            ...
            if (GUILayout.Button(new GUIContent("Delete", "Delete"), graphDeleteButtonStyle)) RemoveGraph(graph);
            GUILayout.EndHorizontal();
        }
        ...
        private void DrawGraphInfoArea(GraphEditor graphEditor)
        {
            ...
            if (graphEditor.infoFadeArea.BeginFade())
            {
                ...
                if (anyNodesNull) Debug.LogError("Some nodes in the graph are null. Please report this error.");
        ...
        private void DrawSceneGUISettings()
        {
            ...
            if (GUILayout.Button("Scan", darkSkin.button)) MenuScan();
        ...
        private TextAsset SaveGraphData(byte[] bytes, TextAsset target = null)
        {
            ...
            else
            {
                ...
                do
                {
                    ...
                    i++;
                } while (File.Exists(projectPath + path));
            }
            ...
            if (serializationSettingsArea.BeginFade())
            {
                ...
                if (GUILayout.Button("Load from file"))
                {
                    ...
                    if (path != "")
                        try
                        {
                            var bytes = AstarSerializer.LoadFromFile(path);
            ...
            if (editorSettingsArea.BeginFade())
            {
                ...
                if (IsJsEnabled())
                {
                    if (GUILayout.Button(new GUIContent("Disable Js Support",
                            "Revert to only enable pathfinding calls from C#"))) DisableJs();
                }
                else
                {
        ...
        private void DrawDebugSettings()
        {
            ...
            if (script.debugMode == GraphDebugMode.SolidColor)
            {
                ...
                if (EditorGUI.EndChangeCheck()) script.colorSettings.PushToStatic(script);
            }
        ...
        private void DrawColorSettings()
        {
            ...
            if (colorSettingsArea.BeginFade())
            {
                ...
                if (customAreaColorsOpen)
                {
                    ...
                    {
                        var newcols = new Color[colors._AreaColors.Length - 1];
                        for (var i = 0; i < colors._AreaColors.Length - 1; i++) newcols[i] = colors._AreaColors[i];
                        colors._AreaColors = newcols;
                    }
                ...
                if (GUI.changed) colors.PushToStatic(script);
            }
        ...
        private void CheckGraphEditors(bool forceRebuild = false)
        {
            ...
            {
                if (script.data.graphs == null) script.data.graphs = new NavGraph[0];
                ...
                for (var i = 0; i < script.graphs.Length; i++)
                {
                    ...
                    if (graph.guid == new Guid()) graph.guid = Guid.NewGuid();
            ...
            else
            {
                for (var i = 0; i < script.graphs.Length; i++)
                {
                    ...
                    if (script.graphs[i].guid == new Guid()) script.graphs[i].guid = Guid.NewGuid();
        ...
        private void AddGraph(Type type)
        {
        ...
        private GraphEditor CreateGraphEditor(NavGraph graph)
        {
            ...
            {
                result = Activator.CreateInstance(graphEditorTypes[graphType].editorType) as GraphEditor;
            }
            else
            {
            ...
            result.infoFadeArea = new FadeArea(graph.infoScreenOpen, this, null);
            result.target = graph;
            result.OnEnable();
            return result;
        }
        ...
        private static int ByteArrayHash(byte[] arr)
        {
            ...
            var hash = -1;
            for (var i = 0; i < arr.Length; i++) hash ^= (arr[i] ^ i) * 3221;
            return hash;
        }
        ...
        public void SaveGraphsAndUndo(EventType et = EventType.Used, string eventCommand = "")
        {
            ...
            if (Application.isPlaying || script.isScanning || script.IsAnyWorkItemInProgress) return;
            ...
            if (Event.current == null || script.data.GetData() == null) SerializeIfDataChanged();
        }
        ...
        private void DeserializeGraphs(byte[] bytes)
        {
            try
            {
                ...
                for (var i = 0; i < graphEditors.Length; i++)
                {
                    ...
                    if (data != null)
                        TinyJsonDeserializer.Deserialize(data, graphEditors[i].GetType(), graphEditors[i],
                            script.gameObject);
                }
            }
            catch (Exception e)
            ...
            try
            {
                ...
                foreach (var p in AstarPath.active.ScanAsync())
                    ...
                    if (Time.realtimeSinceStartup - lastMessageTime > 0.2f)
                    {
                        ...
                        EditorUtility.DisplayProgressBar("Scanning", p.description, p.progress);
        ...
        private void FindGraphTypes()
        {
            ...
            foreach (var assembly in AppDomain.CurrentDomain.GetAssemblies())
            {
                ...
                foreach (var type in types)
                {
                    ...
                    while (!Equals(baseType, null))
                    {
                        if (Equals(baseType, typeof(GraphEditor)))
                        {
                            ...
                            foreach (var attribute in att)
                            {
                                ...
                                if (cge != null && !Equals(cge.graphType, null))
                                {
                                    cge.editorType = type;
