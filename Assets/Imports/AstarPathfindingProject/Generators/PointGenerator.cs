...
namespace Pathfinding
{
	...
	///     Basic point graph.
	///     \ingroup graphs
	///     The point graph is the most basic graph structure, it consists of a number of interconnected points in space called
	///     nodes or waypoints.\n
	///     The point graph takes a Transform object as "root", this Transform will be searched for child objects, every child
	///     object will be treated as a node.
	///     If <see cref="recursive" /> is enabled, it will also search the child objects of the children recursively.
	///     It will then check if any connections between the nodes can be made, first it will check if the distance between
	///     the nodes isn't too large (<see cref="maxDistance)" />
	...
	///     but on the same Y level should still be possible. <see cref="limits" /> and <see cref="maxDistance" /> are treated
	///     as being set to infinity if they are set to 0 (zero). \n
	///     Lastly it will check if there are any obstructions between the nodes using
	///     <a href="http://unity3d.com/support/documentation/ScriptReference/Physics.Raycast.html">raycasting</a> which can
	///     optionally be thick.\n
	///     One thing to think about when using raycasting is to either place the nodes a small
	///     distance above the ground in your scene or to make sure that the ground is not in the raycast mask to avoid the
	///     raycast from hitting the ground.\n
	...
	///     If you have the pro version you can enable <see cref="optimizeForSparseGraph" /> which will in most cases reduce
    ...
    public class PointGraph : NavGraph
    {
	    ...
	    ///     Distance query mode.
	    ///     [Open online documentation to see images]
	    ///     In the image above there are a few red nodes. Assume the agent is the orange circle. Using the Node mode the
	    ///     closest point on the graph that would be found would be the node at the bottom center which
	    ///     may not be what you want. Using the %Connection mode it will find the closest point on the connection between the
	    ///     two nodes in the top half of the image.
	    ///     When using the %Connection option you may also want to use the %Connection option for the Seeker's Start End
	    ///     Modifier snapping options.
	    ...
	    public enum NodeDistanceMode
        {
	        ...
	        ///     This is useful if you have long connections where the agent might be closer to some unrelated node if it is
	        ///     standing on a long connection between two nodes.
        ...
        ///     If you enable this during runtime, you will need to call <see cref="RebuildConnectionDistanceLookup" /> to make
        ///     sure some cache data is properly recalculated.
        ///     If the graph doesn't have any nodes yet or if you are going to scan the graph afterwards then you do not need to do
        ///     this.
        /// </summary>
        ...
        private NNInfoInternal FindClosestConnectionPoint(PointNode node, Vector3 position)
        {
            ...
            if (conns != null)
                ...
                {
                    var connectionMidpoint = ((Vector3)conns[i].node.position + nodePos) * 0.5f;
        ...
        public T AddNode<T>(T node, Int3 position) where T : PointNode
        {
            ...
            {
                var newNodes = new PointNode[nodes != null ? Math.Max(nodes.Length + 4, nodes.Length * 2) : 4];
        ...
        /// <summary>
        ///     Rebuilds a cache used when <see cref="nearestNodeDistanceMode" /> =
        ///     <see cref="NodeDistanceMode.ToConnection" />
        /// </summary>
        ...
        ///     If you are modifying node connections yourself (i.e. manipulating the PointNode.connections array) then you must
        ///     call this function
        ...
        /// </summary>
        /// <param name="sqrLength">
        ///     The length of the connection in squared Int3 units. This can be calculated using
        ///     (node1.position - node2.position).sqrMagnitudeLong.
        /// </param>
        ...
        ///     This is useful if you have created nodes manually using <see cref="AddNode" /> and then want to connect them in the
        ///     same way as the point graph normally connects nodes.
        /// </summary>
        ...
        private IEnumerable<Progress> ConnectNodesAsync()
        {
            if (maxDistance >= 0)
            {
                ...
                for (var i = 0; i < nodeCount; i++)
                {
                    if (i % YieldEveryNNodes == 0) yield return new Progress(i / (float)nodeCount, "Connecting nodes");
            ...
            if (maxDistance == 0 || dist < maxDistance)
            {
                if (raycast)
                {
                    ...
                    if (use2DPhysics)
                    {
                        ...
                        return !Physics2D.Linecast((Vector3)a.position, (Vector3)b.position, mask) &&
                               !Physics2D.Linecast((Vector3)b.position, (Vector3)a.position, mask);
                    }
            ...
            else if (!string.IsNullOrEmpty(searchTag))
            {
                ...
                for (var i = 0; i < gos.Length; i++)
                    Gizmos.DrawCube(gos[i].transform.position,
                        Vector3.one * HandleUtility.GetHandleSize(gos[i].transform.position) * 0.1F);
            }
        }
