...
namespace Pathfinding
{
    ...
    public abstract class GridNodeBase : GraphNode
    {
        ...
        {
            get => nodeInGridIndex & NodeInGridIndexMask;
        ...
        {
            get => (gridFlags & GridFlagsWalkableErosionMask) != 0;
            set
            {
                unchecked
                {
                    gridFlags = (ushort)((gridFlags & ~GridFlagsWalkableErosionMask) |
                                         (value ? GridFlagsWalkableErosionMask : 0));
                }
            }
        }
        ...
        {
            get => (gridFlags & GridFlagsWalkableTmpMask) != 0;
            set
            {
                unchecked
                {
                    gridFlags = (ushort)((gridFlags & ~GridFlagsWalkableTmpMask) |
                                         (value ? GridFlagsWalkableTmpMask : 0));
                }
            }
        }
...
#else
	    /// <summary>
	    ///     Same as <see cref="ClearConnections" />, but does not clear grid connections, only custom ones (e.g added by
	    ///     <see cref="AddConnection" /> or a NodeLink component)
	    /// </summary>
	    public void ClearCustomConnections(bool alsoReverse)
        {
            if (connections != null)
