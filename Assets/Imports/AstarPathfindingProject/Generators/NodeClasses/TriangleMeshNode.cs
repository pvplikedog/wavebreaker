...
namespace Pathfinding
{
    ...
    public class TriangleMeshNode : MeshNode
    {
        ...
        ///     This number does not necessarily need to be a index in an array but two different vertices (in the same graph)
        ///     should
        ...
        ///     - It is guaranteed to be inside the node, so if you call <see cref="ContainsPointInGraphSpace" /> with the return
        ///     value from this method the result is guaranteed to be true.
        ...
        public override bool ContainsPointInGraphSpace(Int3 p)
        {
            ...
            if ((b.x - a.x) * (long)(p.z - a.z) - (p.x - a.x) * (long)(b.z - a.z) > 0) return false;

            if ((c.x - b.x) * (long)(p.z - b.z) - (p.x - b.x) * (long)(c.z - b.z) > 0) return false;

            if ((a.x - c.x) * (long)(p.z - c.z) - (p.x - c.x) * (long)(a.z - c.z) > 0) return false;
        ...
        public override void Open(Path path, PathNode pathNode, PathHandler handler)
        {
            ...
            for (var i = connections.Length - 1; i >= 0; i--)
            {
                ...
                if (path.CanTraverse(conn.node))
                {
                    ...
                    if (pathOther == pathNode.parent) continue;
        ...
        ///     If there is a connection with the other node, but the connection is not marked as using a particular edge of the
        ///     shape of the node
        ...
        /// </code>
        ///     See: <see cref="GetPortal" /> which also handles edges that are shared over tile borders and some types of node
        ///     links
        /// </summary>
        ...
        public bool GetPortal(GraphNode toNode, List<Vector3> left, List<Vector3> right, bool backwards, out int aIndex,
            out int bIndex)
        {
            aIndex = -1;
            bIndex = -1;
            ...
            if (tileIndex1 != tileIndex2)
            {
                ...
                if (Math.Abs(x1 - x2) == 1) coord = 2;
                else if (Math.Abs(z1 - z2) == 1) coord = 0;
                ...
                if (otherEdge != -1)
                {
                    ...
                    var mincoord = Math.Min(v1a[coord], v1b[coord]);
                    var maxcoord = Math.Max(v1a[coord], v1b[coord]);
                    ...
                    mincoord = Math.Max(mincoord, Math.Min(v2a[coord], v2b[coord]));
                    maxcoord = Math.Min(maxcoord, Math.Max(v2a[coord], v2b[coord]));
        ...
        public override float SurfaceArea()
        {
            ...
            return Math.Abs(VectorMath.SignedTriangleAreaTimes2XZ(holder.GetVertex(v0), holder.GetVertex(v1),
