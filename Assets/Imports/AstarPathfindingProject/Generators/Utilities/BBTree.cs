...
namespace Pathfinding
{
	...
	public class BBTree : IAstarPooledObject
    {
        ...
        public void RebuildFrom(TriangleMeshNode[] nodes)
        {
            ...
            for (var i = 0; i < nodes.Length; i++) permutation[i] = i;
        ...
        private int RebuildFromInternal(TriangleMeshNode[] nodes, int[] permutation, IntRect[] nodeBounds, int from,
            int to, bool odd)
        {
        ...
        private void SearchBoxClosestXZ(int boxi, Vector3 p, ref float closestSqrDist, NNConstraint constraint,
        ...
        private void SearchBoxClosest(int boxi, Vector3 p, ref float closestSqrDist, NNConstraint constraint,
            ref NNInfoInternal nnInfo)
        {
            var box = tree[boxi];
            ...
            else
            {
                ...
                // Search children (closest box first to improve performance)
                if (firstDist < closestSqrDist) SearchBoxClosest(first, p, ref closestSqrDist, constraint, ref nnInfo);
        ...
        private void GetOrderedChildren(ref int first, ref int second, out float firstDist, out float secondDist,
            Vector3 p)
        {
            ...
            if (box.IsLeaf)
            {
                ...
                for (var i = 0; i < MaximumLeafSize && nodes[box.nodeOffset + i] != null; i++)
                {
                    ...
                    if (node.ContainsPoint((Int3)p))
                    {
                        ...
                        if (constraint == null || constraint.Suitable(node)) return node;
                    }
                    else
                    {
                        DrawDebugNode(node, 0.0f, Color.blue);
