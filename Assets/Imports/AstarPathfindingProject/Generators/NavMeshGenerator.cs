...
namespace Pathfinding
{
    ...
    ///     Polygons instead of points mean that the funnel smoother can produce really nice looking paths and the graphs are
    ///     also really fast to search
    ///     and have a low memory footprint because fewer nodes are usually needed to describe the same area compared to grid
    ///     graphs.
    ...
    public class NavMeshGraph : NavmeshBase, IUpdatableGraph
    {
	    ...
	    ///     This is important when the graph has been saved to a file and is later loaded again, but the original mesh does not
	    ///     exist anymore (or has been moved).
	    ///     In that case we still need to be able to find the bounding box since the <see cref="CalculateTransform" /> method
	    ///     uses it.
	    /// </summary>
        ...
        ///     The normals are important when connecting adjacent nodes. Two adjacent nodes will only be connected if they are
        ///     oriented the same way.
        ///     This is particularly important if you have a navmesh on the walls or even on the ceiling of a room. Or if you are
        ///     trying to make a spherical navmesh.
        ///     If you do one of those things then you should set disable this setting and make sure the normals in your source
        ///     mesh are properly set.
        ...
        ///     that
        ///     has all normals pointing downwards and this option is false, then some methods like <see cref="PointOnNavmesh" />
        ///     will not work correctly
        ///     as they assume that the normals point upwards. For a more complicated surface like a spherical graph those methods
        ...
        protected override bool RecalculateNormals => recalculateNormals;
        ...
        public static void UpdateArea(GraphUpdateObject o, INavmeshHolder graph)
        {
            ...
            graph.GetNodes(_node =>
            {
                ...
                if (!inside && (allLeft == 3 || allRight == 3 || allTop == 3 || allBottom == 3)) return;
                ...
                if (inside || node.ContainsPointInGraphSpace(a) || node.ContainsPointInGraphSpace(b) ||
                    node.ContainsPointInGraphSpace(c) || node.ContainsPointInGraphSpace(d)) inside = true;
