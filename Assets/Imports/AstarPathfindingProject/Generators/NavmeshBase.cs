...
namespace Pathfinding
{
    ...
    public abstract class NavmeshBase : NavGraph, INavmesh, INavmeshHolder, ITransformedGraph
    {
        ...
        private readonly List<int> batchUpdatedTiles = new();

        /// <summary>List of nodes that are going to be destroyed as part of a batch update</summary>
        private readonly List<MeshNode> batchNodesToDestroy = new();
        ...
        ///     Moves all the nodes in such a way that the specified transform is the new graph space to world space transformation
        ///     for the graph.
        ...
        /// <param name="rect">
        ///     Graph space rectangle (in graph space all tiles are on the XZ plane regardless of graph rotation and
        ///     other transformations, the first tile has a corner at the origin)
        /// </param>
        ...
        private static readonly NNConstraint NNConstraintDistanceXZ = new() { distanceXZ = true };
        ...
        protected void ConnectTiles(NavmeshTile tile1, NavmeshTile tile2)
        {
            ...
            if (tile1.nodes == null) throw new ArgumentException("tile1 does not contain any nodes");
            if (tile2.nodes == null) throw new ArgumentException("tile2 does not contain any nodes");
            ...
            if (Math.Abs(t1coord - t2coord) != 1)
                throw new ArgumentException("Tiles are not adjacent (tile coordinates must differ by exactly 1. Got '" +
                                            t1coord + "' and '" + t2coord + "')");
        ...
        {
            if (!batchTileUpdate) throw new Exception("Must be called during a batch update. See StartBatchTileUpdate");
        ...
        private readonly Dictionary<int, int> nodeRecyclingHashBuffer = new();
        ...
        ///     This will create new nodes and link them to the adjacent tile (unless batching has been started in which case that
        ///     will be done when batching ends).
        ...
        private void CreateNavmeshSurfaceVisualization(NavmeshTile[] tiles, int startTile, int endTile,
        ...
        private static void CreateNavmeshOutlineVisualization(NavmeshTile[] tiles, int startTile, int endTile,
            GraphGizmoHelper helper)
        {
        ...
        protected override void SerializeExtraInfo(GraphSerializationContext ctx)
        {
            ...
            for (var x = 0; x < tileXCount; x++)
            {
                ...
                if (tile == null) throw new Exception("NULL Tile");
                //writer.Write (-1);
                //continue;
                writer.Write(tile.x);
                writer.Write(tile.z);
        ...
        protected override void DeserializeExtraInfo(GraphSerializationContext ctx)
        {
            ...
            for (var x = 0; x < tileXCount; x++)
            {
                ...
                if (tx < 0) throw new Exception("Invalid tile coordinates (x < 0)");

                var tz = reader.ReadInt32();
                if (tz < 0) throw new Exception("Invalid tile coordinates (z < 0)");
        ...
        protected override void PostDeserialization(GraphSerializationContext ctx)
        {
            ...
            {
                var conns = tiles.SelectMany(s => s.nodes)
