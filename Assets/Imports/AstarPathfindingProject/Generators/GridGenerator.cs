...
namespace Pathfinding
{
	...
	///     [Open online documentation to see images]
	///     The <b>The Snap Size</b> button snaps the internal size of the graph to exactly contain the current number of
	///     nodes, i.e not contain 100.3 nodes but exactly 100 nodes.\n
	...
	///     For grid graphs this is a great feature since you can update only a small part of the grid without causing any lag
	///     like a complete rescan would.\n
	///     If you for example just have instantiated a sphere obstacle in the scene and you want to update the grid where that
	///     sphere was instantiated, you can do this:\n
	///     <code> AstarPath.active.UpdateGraphs (ob.collider.bounds); </code>
	///     Where ob is the obstacle you just instantiated (a GameObject).\n
	///     As you can see, the UpdateGraphs function takes a Bounds parameter and it will send an update call to all
	///     updateable graphs.\n
	///     A grid graph will update that area and a small margin around it equal to \link Pathfinding.GraphCollision.diameter
	///     collision testing diameter/2 \endlink
	///     See: graph-updates (view in online documentation for working links) for more info about updating graphs during
	///     runtime
	///     <b>Hexagon graphs</b>\n
	///     The graph can be configured to work like a hexagon graph with some simple settings. Since 4.1.x the grid graph has
	///     a 'Shape' dropdown.
	...
	///     See: <see cref="Pathfinding.GraphCollision" /> for documentation on the 'Height Testing' and 'Collision Testing'
	///     sections
    ...
    public class GridGraph : NavGraph, IUpdatableGraph, ITransformedGraph
    {
        ...
        [NonSerialized] public readonly uint[] neighbourCosts = new uint[8];
        ...
        /// </code>
        /// </summary>
        [NonSerialized] public readonly int[] neighbourOffsets = new int[8];

        /// <summary>Offsets in the X direction for neighbour nodes. Only 1, 0 or -1</summary>
        [NonSerialized] public readonly int[] neighbourXOffsets = new int[8];

        /// <summary>Offsets in the Z direction for neighbour nodes. Only 1, 0 or -1</summary>
        [NonSerialized] public readonly int[] neighbourZOffsets = new int[8];
        ...
        ///     Tags will be used for erosion instead of marking nodes as unwalkable. The nodes will be marked with tags in an
        ///     increasing order starting with the tag <see cref="erosionFirstTag" />.
        ///     Debug with the Tags mode to see the effect. With this enabled you can in effect set how close different AIs are
        ///     allowed to get to walls using the Valid Tags field on the Seeker component.
        ///     [Open online documentation to see images]
        ///     [Open online documentation to see images]
        ///     See: erosionFirstTag
        /// </summary>
        ...
        ///     For hexagons the normal nodeSize field doesn't really correspond to anything specific on the hexagon's geometry, so
        ///     this enum is used to give the user the opportunity to adjust more concrete dimensions of the hexagons
        ///     without having to pull out a calculator to calculate all the square roots and complicated conversion factors.
        ///     This field is only used in the graph inspector, the <see cref="nodeSize" /> field will always use the same internal
        ///     units.
        ...
        ///     If you are making a 2D isometric game, you may want to use this parameter to adjust the layout of the graph to
        ///     match your game.
        ...
        ///     Usually the angle that you want to use is either 30 degrees (alternatively 90-30 = 60 degrees) or atan(1/sqrt(2))
        ///     which is approximately 35.264 degrees (alternatively 90 - 35.264 = 54.736 degrees).
        ///     You might also want to rotate the graph plus or minus 45 degrees around the Y axis to get the oritientation
        ...
        /// <summary>
        ///     Show the surface of the graph. Each node will be drawn as a square (unless e.g hexagon graph mode has been
        ///     enabled).
        /// </summary>
        ...
        ///     It is used mainly in the editor to do auto-scanning calls, setting it to false for a non-uniform grid will reduce
        ///     the number of scans
        /// </summary>
        ...
        /// </summary>
        protected bool useRaycastNormal => Math.Abs(90 - maxSlope) > float.Epsilon;
        ...
        ///     Note: This is just a convenience property, this property will actually read/modify the <see cref="rotation" /> of
        ///     the graph. A rotation aligned with the 2D plane is what determines if the graph is 2D or not.
        ///     See: You can also set if the graph should use 2D physics using `this.collision.use2D`
        ///     (\reflink{GraphCollision.use2D}).
        /// </summary>
        public bool is2D
        {
            get => Quaternion.Euler(rotation) * Vector3.up == -Vector3.forward;
            set
            {
                if (value != is2D)
                    rotation = value ? new Vector3(rotation.y - 90, 270, 90) : new Vector3(0, rotation.x + 90, 0);
            }
        }
        ...
        {
            if (mode == InspectorGridHexagonNodeSize.Diameter) value *= 1.5f / (float)Math.Sqrt(2.0f);
            else if (mode == InspectorGridHexagonNodeSize.Width) value *= (float)Math.Sqrt(3.0f / 2.0f);
            return value;
        }

        public static float ConvertNodeSizeToHexagonSize(InspectorGridHexagonNodeSize mode, float value)
        {
            if (mode == InspectorGridHexagonNodeSize.Diameter) value *= (float)Math.Sqrt(2.0f) / 1.5f;
            else if (mode == InspectorGridHexagonNodeSize.Width) value *= (float)Math.Sqrt(2.0f / 3.0f);
            return value;
        }
        ...
        public GridNode GetNodeConnection(GridNode node, int dir)
        {
            ...
            if (!node.EdgeNode) return nodes[node.NodeInGridIndex + neighbourOffsets[dir]];
        ...
        ///     Calling this method will set <see cref="isometricAngle" />, <see cref="aspectRatio" />,
        ///     <see cref="uniformEdgeCosts" /> and <see cref="neighbours" />
        ///     to appropriate values for that shape.
        ///     Note: Setting the shape to <see cref="InspectorGridMode.Advanced" /> does not do anything except set the
        ///     <see cref="inspectorGridMode" /> field.
        ...
        /// <summary>
        ///     Updates <see cref="unclampedSize" /> from <see cref="width" />, <see cref="depth" /> and <see cref="nodeSize" />
        ///     values.
        ...
        /// <summary>
        ///     Updates <see cref="unclampedSize" /> from <see cref="width" />, <see cref="depth" /> and
        ///     <see cref="nodeSize" /> values. Deprecated: Use <see cref="SetDimensions" /> instead
        /// </summary>
        ...
        {
            if (nodes == null || depth * width != nodes.Length) return new NNInfoInternal();
        ...
        public override NNInfoInternal GetNearestForce(Vector3 position, NNConstraint constraint)
        {
            ...
            var maxDist = constraint == null || constraint.constrainDistance
        ...
        /// <summary>
        ///     Sets up <see cref="neighbourOffsets" /> with the current settings. <see cref="neighbourOffsets" />,
        ///     <see cref="neighbourCosts" />, <see cref="neighbourXOffsets" /> and <see cref="neighbourZOffsets" /> are set up.\n
        ///     The cost for a non-diagonal movement between two adjacent nodes is RoundToInt (<see cref="nodeSize" /> *
        ///     Int3.Precision)\n
        ///     The cost for a diagonal movement between two adjacent nodes is RoundToInt (<see cref="nodeSize" /> * Sqrt (2) *
        ///     Int3.Precision)
        /// </summary>
        public virtual void SetUpOffsetsAndCosts()
        {
            ...
            var diagonalCost = uniformEdgeCosts
                ? straightCost
        ...
        protected override IEnumerable<Progress> ScanInternal()
        {
            ...
            if (collision == null) collision = new GraphCollision();
            collision.Initialize(transform, nodeSize);
        ...
        /// <param name="resetPenalties">
        ///     If true, the penalty of the nodes will be reset to the initial value as if the graph had just been scanned
        ...
        public virtual void RecalculateCell(int x, int z, bool resetPenalties = true, bool resetTags = true)
        {
            ...
            if (walkable && useRaycastNormal && collision.heightCheck)
                if (hit.normal != Vector3.zero)
                {
                    ...
                    if (angle < cosAngle) walkable = false;
                }
        ...
        {
            if (node.Walkable && ErosionAnyFalseConnections(node)) node.Walkable = false;
        }
        ...
        public void ErodeWalkableArea(int xmin, int zmin, int xmax, int zmax)
        {
            ...
            for (var it = 0; it < erodeIterations; it++)
                ...
                else
                {
                    ...
                    for (var i = 0; i < nodeCount; i++) ErodeNode(nodesInRect[i]);

                    for (var i = 0; i < nodeCount; i++) CalculateConnections(nodesInRect[i] as GridNodeBase);
                }
        ...
        {
            if (!node1.Walkable || !node2.Walkable) return false;
            ...
            if (transform.onlyTranslational)
                ...
                return Math.Abs(node1.position.y - node2.position.y) <= maxClimb * Int3.Precision;

            var p1 = (Vector3)node1.position;
            var p2 = (Vector3)node2.position;
            var up = transform.WorldUpAtGraphPosition(p1);
            return Math.Abs(Vector3.Dot(up, p1) - Vector3.Dot(up, p2)) <= maxClimb;
        }
        ...
        public void CalculateConnectionsForCellAndNeighbours(int x, int z)
        {
            ...
            for (var i = 0; i < 8; i++)
            {
                ...
                if ((nx >= 0) & (nz >= 0) & (nx < width) & (nz < depth)) CalculateConnections(nx, nz);
            }
        }
                        ...
                        for (var i = 0; i < 4; i++)
                            ...
                            if ((((conns >> i) | (conns >> (i + 1)) | (conns >> (i + 1 - 4))) & 1) != 0)
                            {
                                ...
                                if ((nx >= 0) & (nz >= 0) & (nx < width) & (nz < depth))
                                {
                                    ...
                                    if (IsValidConnection(node, other)) diagConns |= 1 << directionIndex;
                                }
                            }
                    }
                    else
                    {
                        for (var i = 0; i < 4; i++)
                            ...
                            if (((conns >> i) & 1) != 0 && (((conns >> (i + 1)) | (conns >> (i + 1 - 4))) & 1) != 0)
                            {
                                ...
                                if (IsValidConnection(node, other)) diagConns |= 1 << (i + 4);
                            }
                    }
                }
        ...
        public override void OnDrawGizmos(RetainedGizmos gizmos, bool drawNodes)
        {
            ...
            for (var cz = depth / chunkWidth; cz >= 0; cz--)
            {
                ...
                for (var i = 0; i < allNodesCount; i++) hasher.HashNode(allNodes[i]);
                Profiler.EndSample();
        ...
        private void CreateNavmeshSurfaceVisualization(GridNodeBase[] nodes, int nodeCount, GraphGizmoHelper helper)
        {
            ...
            for (var i = 0; i < nodeCount; i++)
            {
                ...
                for (var dIndex = 0; dIndex < neighbourIndices.Length; dIndex++)
                {
                    ...
                    if (n1 != null && neighbours != NumNeighbours.Six) n3 = n1.GetNeighbourAlongDirection(nextD);
                ...
                for (var j = 0; j < verticesPerNode; j++) colors[baseIndex + j] = nodeColor;
        ...
        protected virtual List<GraphNode> GetNodesInRegion(Bounds bounds, GraphUpdateShape shape)
        {
            ...
            if (nodes == null || !rect.IsValid() || nodes.Length != width * depth) return ListPool<GraphNode>.Claim();
            ...
            for (var z = rect.ymin; z <= rect.ymax; z++)
            {
                ...
                if (bounds.Contains((Vector3)node.position) &&
                    (shape == null || shape.Contains((Vector3)node.position))) inArea.Add(node);
            }
        ...
        public virtual List<GraphNode> GetNodesInRegion(IntRect rect)
        {
            ...
            {
                var zw = z * Width;
                for (var x = rect.xmin; x <= rect.xmax; x++) inArea.Add(nodes[zw + x]);
            }
        ...
        ///     Note: This method is much faster than GetNodesInRegion(IntRect) which returns a list because this method can make
        ...
        /// <param name="rect">Region in which to return nodes. It will be clamped to the grid.</param>
        /// <param name="buffer">
        ///     Buffer in which the nodes will be stored. Should be at least as large as the number of nodes that
        ...
        public virtual int GetNodesInRegion(IntRect rect, GridNodeBase[] buffer)
        {
            ...
            for (var z = rect.ymin; z <= rect.ymax; z++, counter += rect.Width)
                Array.Copy(nodes, z * Width + rect.xmin, buffer, counter, rect.Width);
        ...
        ///     This will also return true if <see cref="neighbours" /> = NumNeighbours.Four, the direction is diagonal and one can
        ///     move through one of the adjacent nodes
        ...
        public bool CheckConnection(GridNode node, int dir)
        {
            ...
            if (!HasNodeConnection(node, dir1) || !HasNodeConnection(node, dir2)) return false;

            var n1 = nodes[node.NodeInGridIndex + neighbourOffsets[dir1]];
            var n2 = nodes[node.NodeInGridIndex + neighbourOffsets[dir2]];

            if (!n1.Walkable || !n2.Walkable) return false;

            if (!HasNodeConnection(n2, dir1) || !HasNodeConnection(n1, dir2)) return false;
            return true;
        }
        ...
        protected override void SerializeExtraInfo(GraphSerializationContext ctx)
        {
            ...
            for (var i = 0; i < nodes.Length; i++) nodes[i].SerializeNode(ctx);
        }
